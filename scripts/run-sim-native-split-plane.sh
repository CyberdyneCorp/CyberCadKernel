...
HARNESS="$REPO/tests/sim/native_split_plane_parity.mm"
...
# definition lives in src/native/boolean/ssi_boolean.cpp (the only non-header native TU
# the DM1 verb adds beyond the freeform harness's set).
NATIVE_SRCS=()
...
echo "── compiling B4 DM1 split-plane native-vs-OCCT parity harness (iphonesimulator arm64)"
...
xcrun --sdk iphonesimulator clang++ -target arm64-apple-ios16.0-simulator -isysroot "$SYSROOT" \
  ...
  -o "$OUT/native_split_plane_parity"
...
xcrun simctl spawn "$UDID" "$OUT/native_split_plane_parity"
