#!/usr/bin/env bash
# Compile the MOAT M2 chain-seam STRIP WELD (multi-face two-junction weld)
...
HARNESS="$REPO/tests/sim/native_chain_seam_weld_parity.mm"
...
echo "── compiling M2-multiseam chain-seam strip weld native-vs-OCCT parity harness (iphonesimulator arm64)"
...
xcrun --sdk iphonesimulator clang++ -target arm64-apple-ios16.0-simulator -isysroot "$SYSROOT" \
  ...
  -o "$OUT/native_chain_seam_weld_parity"
...
xcrun simctl spawn "$UDID" "$OUT/native_chain_seam_weld_parity"
