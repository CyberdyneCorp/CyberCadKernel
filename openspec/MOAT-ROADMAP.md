...
- **Status — UNEQUAL-radius CYL↔CYL CANAL fillet LANDED (`moat-m3uc-unequal-canal-fillet`) → the
  FIRST entry of the equal-canal tail now NATIVE.** `cc_fillet_edges` now LANDS on the two crossing
  creases of two ORTHOGONAL-axis cylinders of DISTINCT radii (the thin cyl poking fully through the
  thick one): `src/native/blend/canal_fillet_unequal.h` (new). Distinct radii change the crease
  topology in the MORE REGULAR direction — for thin cyl A (axis Z, `Ra`) and thick cyl B (axis X,
  `Rb`), `Ra<Rb`, the walls meet along `cz=±√(Rb²−Ra² sin²u)`, and since `Rb>Ra` the radicand
  `≥ Rb²−Ra²>0` for EVERY azimuth, so `cz` never reaches 0: the crease is TWO DISJOINT CLOSED LOOPS,
  NOT the equal-case pinched figure sharing poles. Each loop gets a NON-DEGENERATE closed canal strip
  (rolling-ball centre at `R0a=Ra−r` from the thin axis and `R0b=Rb−r` from the thick axis, never a
  pole), G1-tangent to both walls by construction (the strip normal `(P−C)/r` IS the wall radial at
  each seam — verified analytically). The whole filleted COMMON rebuilds as a planar-facet soup —
  two closed strips + the thin wall's waist TUBE band + two thick-wall CAP patches (ring-fanned from
  the loop centroid on the thick wall) — welded watertight PURELY in the assembly layer (**NO
  tessellator change** — byte-identical firewall trivially met, zero mesher diff). Recognition is
  WHOLESALE from the boolean's facet soup, recovering the two axes and TWO DISTINCT radii classified
  PER FACET BY RADIUS (a facet on the `Ra` cylinder sits at perp-distance `Ra` EXACTLY regardless of
  its facet-normal tilt — robust to coarse-cap tilt a hard normal threshold rejects). Candidate #7 in
  `NativeEngine::fillet_edges` (after the equal canal #6), gated by the SHRINK self-verify; EQUAL
  radii route to #6; a MANDATORY internal self-verify (consistent orientation + removed-volume bound)
  rejects any large-radius fold → OCCT. GATE a host-analytic GREEN (`test_native_blend`
  `unequal_canal_fillet_*` 5 cases: `Ra=1,Rb=1.5` fillets at `r∈[0.1,0.4]` watertight +
  `isConsistentlyOriented` (χ=2) + REMOVES material + CONVERGES as deflection refines; ANALYTIC G1 at
  both seams; box / equal-radii / thin `Ra<2r` / `r≤0` / multi-edge decline). GATE b SIM encoded
  structurally (OCCT `BRepPrimAPI_MakeCylinder`×2 + `Common` + `BRepFilletAPI_MakeFillet` oracle).
  SCOPE (honest): EQUAL radii → canal_fillet.h; non-orthogonal / non-crossing / a third wall family
  / TORUS / elliptical creases still decline → OCCT. BODY-BUILD CAVEAT (measured): the native SSI
  boolean does build the unequal COMMON but impractically (~48 s, ~59k facets vs 864 equal) — a
...
- **Breadth — 14th native domain: SHAPE-HEALING (deliberately-defected soups vs OCCT
  ShapeFix).** `tests/sim/native_healing_fuzz.mm` +
  `scripts/run-sim-native-healing-fuzz.sh`: a DETERMINISTIC seeded generator (splitmix64→
  xoshiro256**, seeded ONLY by FUZZ_SEED) builds a random VALID base solid whose exact
  geometry is KNOWN (unit cube V=1 / random axis-aligned box / random convex N-gon prism →
  ...
  BOTH ways — the OCCT-FREE native `heal::healShell` (with the family opt-in flag) and the
  OCCT oracle `cyber::occt::sewAndFix` (`BRepBuilderAPI_Sewing`→`ShapeFix_Shell`/`_Solid`,
  ...
  so the base solid's closed-form volume+area is exact ground truth — a native `Healed`
  ...
  (native declining where OCCT aggressively repairs to the SAME honest solid — the
  short-edge/collinear flag-OFF probes — is AGREED-by-more-conservative, never a fault).
  Two seeds (`0x4845414C4F` N=120 → 79 AGREED / 3 HONESTLY-DECLINED / 0 DISAGREED / 0
  ORACLE-INACCURATE / 38 BOTH-DECLINED / 0 ORACLE_UNRELIABLE; `0xC0FFEEBEEF11` N=120 → 87 /
  2 / 0 / 0 / 31 / 0): **0 DISAGREED** on both seeds, all three base families and all nine
  heal-expected defect families with ≥1 AGREED, all three decline-probe families
  ...
  first-class HONESTLY-DECLINED (native → OCCT), never DISAGREED. Determinism re-verified
  (same seed → byte-identical batch). `src/native` / `src/engine` / `include` / the `cc_*`
  ABI byte-unchanged; on run-sim-suite.sh SKIP list (own main(), `std::_Exit`). OpenSpec
  change `moat-m6n-healing-fuzz`.
- **Breadth — 16th native domain: DRAFT ANGLE (taper planar side faces through the `cc_*`
  facade under BOTH engines).** `tests/sim/native_draft_faces_fuzz.mm` +
  `scripts/run-sim-native-draft-faces-fuzz.sh`: a DETERMINISTIC seeded generator (splitmix64→
  xoshiro256**, seeded ONLY by FUZZ_SEED) builds a random VALID prismatic solid (BOX / regular
  NGON prism, n∈[3,8]) IDENTICALLY under both engines via `cc_solid_extrude`, resolves a random
  ...
  under BOTH `cc_set_engine(1)` (NativeEngine `feature::draftFaces` — each drafted plane derived
  from the original face, applied as an inward `splitByPlane` half-space cut, composite self-
  verified watertight/χ=2/oriented/strict-shrink → OCCT on decline; numsci-gated, so the harness
  links numsci) AND `cc_set_engine(0)` (the OCCT `BRepOffsetAPI_DraftAngle` oracle). The PRIMARY
  arbiter is a CLOSED-FORM drafted volume — the base footprint polygon clipped by each drafted
  edge's inward-shifted (`z·tanθ`) supporting half-plane, integrated exactly over the extrude
  height by 3-point Simpson (A(z) is degree ≤2 in z), handling adjacent-face corner interactions
  EXACTLY (reproduces the parity harness's box wedge `1000−500·tan8°` + 4-side frustum). This is
  the FIRST fuzz domain for the draft op (`native_draft_faces_parity` had 3 hand-picked fixtures
  + 1 decline, no fuzz). The four families are {BOX,NGON}×{single-face,multi-face} compared on
  volume/area/watertight/Euler χ=2/strict-shrink. Two seeds (0xD4AF7A11EE N=72 → 60 AGREED / 12
  HONESTLY-DECLINED / 0 DISAGREED / 0 ORACLE-INACCURATE / 0 ORACLE_UNRELIABLE / 0 BOTH-DECLINED;
  0x5EEDDA7A16 N=72 → 64 / 8 / 0 / 0 / 0 / 0): **0 DISAGREED** on both seeds, each of the four
  families with ≥1 AGREED, every AGREE native==OCCT==closed-form to ~1e-16 (planar draft volume
  ...
  poses (adjacent tilted half-space cuts whose corner the composite self-verify rejects →
  ResolveFailed → OCCT) — first-class equal-or-more-conservative behaviour, confirmed by the
  closed form and OCCT agreeing EXACTLY (`volO==volX`) in every declined case; the obtuse-corner
  NGON multi family seats readily (AGREES on 5/7 and 4/5 drafted faces at 11°). Determinism
  re-verified (same seed → byte-identical batch). `src/native` / `src/engine` / `include` / the
  `cc_*` ABI byte-unchanged; on run-sim-suite.sh SKIP list (own main(), `std::_Exit`). OpenSpec
  change `moat-m6p-draft-fuzz`.
- **Breadth — WRAP-EMBOSS FREEFORM-BASE domain (moat-m6v — closes the FREEFORM sphere-cap gap
  the cylinder-only `native_wrap_emboss_fuzz.mm` left).** `tests/sim/native_wrap_emboss_freeform_fuzz.mm`
  + `scripts/run-sim-native-wrap-emboss-freeform-fuzz.sh`: a DETERMINISTIC seeded generator
  (splitmix64→xoshiro256**, seeded ONLY by `FUZZ_SEED`) drives random VALID wrap-emboss poses
  for the four base×mode cells {cylinder, sphere-cap} × {raised(boss=1), recessed(boss=0)} —
  certifying the NEW F5 arm of `src/native/feature/wrap_emboss.h`, a RAISED circular pole boss
  on a FREEFORM sphere-cap dome (a NON-developable base), alongside the cylinder base as a
  developable control — through the SHIPPING `cc_wrap_emboss` facade under BOTH engines
  (`cc_set_engine(0)`=OCCT, `cc_set_engine(1)`=NativeEngine). The base solid is built
  IDENTICALLY under both engines (`cc_solid_extrude_profile` full circle / `cc_solid_revolve_profile`
  dome); the wrap face is resolved GEOMETRICALLY on the body being embossed. **OCCT's own
  `cc_wrap_emboss` CANNOT wrap a non-cylindrical face** — presented with a sphere wall it
  DECLINES (returns 0, asserted per trial: 23/23 and 15/15 over the two seeds), so for the
  SPHERE base the CLOSED-FORM shell-sector delta `2π(1−cosφ0)·((R+h)³−R³)/3` is the SOLE
  arbiter, added to the base-dome volume OCCT DOES measure exactly (`cc_mass_properties`/
  BRepGProp — an independent OCCT reference for the BASE, not the wrap); for the CYLINDER base
  the closed-form curvature-corrected `A·|Rout²−R²|/(2R)` is PRIMARY with OCCT (the same facade
  under engine 0) a SECONDARY vol+area cross-check. Each in-scope native boss is additionally
  checked watertight + Euler χ=2 + strictly-grown volume (raise) + mesh-vol≈brep-vol. Sparse
  out-of-envelope exercisers (box base, >2π footprint, deboss depth ≥ R, self-intersecting
  pentagram, boss reaching the dome rim) + the sphere-RECESSED cell (an honest DOMAIN-level
  decline — the F5 arm is boss=1 only) route to the native NULL → OCCT / BOTH-DECLINED branch,
  counted separately. Two seeds (0x5745E6F00B N=64 → 47 AGREED / 3 HONESTLY-DECLINED / 0
  DISAGREED / 0 ORACLE_UNRELIABLE / 14 BOTH-DECLINED, per-cell cyl-raised 13 / cyl-recessed 17
  / sphere-raised 17; 0xB0551FEED2 N=64 → 41 / 5 / 0 / 0 / 18, per-cell 16 / 15 / 10):
  **0 DISAGREED** on both seeds, each in-scope cell with ≥1 AGREED, OCCT declined every sphere
  wall (closed-form sole arbiter confirmed), max native-vs-oracle bias cylVol=7.36e-3
  cylArea=1.42e-2 sphVol=4.57e-3 (all comfortably under the FIXED never-widened tol cylVol=2e-2
  cylArea=3e-2 sphVol=1.5e-2 meshRel=2e-2). Determinism re-verified (same seed → byte-identical
  batch). `src/native` / `src/engine` / `include` / the `cc_*` ABI byte-unchanged; on
  run-sim-suite.sh SKIP list (own main(), `std::_Exit`). OpenSpec change `moat-m6v-wrap-emboss-fuzz`.
...
- **construct — VARIABLE-SECTION SWEEP — `cc_variable_sweep` (guide+spine morphing boss)** ✅ *(straight + smooth-planar slice landed — native)*
  ...
  substrate is byte-frozen; the new builder only calls it). The COUPLED morph×scale case (section
  MORPHS A→B AND the guide scale varies together) is now NATIVE too (`moat-vsfix-coupled-morph`): the
  straight-spine guided builder DENSIFIES to `≥3` stations bounded by the coupled area-integral
  (`straightCoupledStations` — the section-radius law `g(f)=morph(f)·scale(f)` is a non-linear PRODUCT
  whose `∫g²` volume a 2-station linear ruling dropped; the two exact sub-regimes stay at 2 stations,
  byte-identical), so the enclosed volume converges to the exact `∫area df`. `native_engine.cpp::
  variable_sweep` keeps a native result only when it self-verifies robustly watertight + positive
  volume, else forwards to `OcctEngine::variable_sweep` = the `BRepOffsetAPI_MakePipeShell`
  MULTI-SECTION + `BRepGProp` oracle (per-station morphed+scaled section wires). GATE (a) HOST 10/10
  (`test_native_vsweep` — circle→circle straight = TRUNCATED CONE `πH/3·(r0²+r0r1+r1²)`, constant
  section = prism, guide-scaled square = FRUSTUM `(H/3)(A0+A1+√(A0A1))`, **COUPLED morph×scale straight
  = the exact polygon-clip integral `C·H·∫₀¹g(f)²df` — was 19% off with the 2-station ruling, now
  converges**, curved-arc morph watertight + χ=2 volume-stable, + declines mismatched-counts /
  coincident-guide / collapsing-guide / degenerate). GATE (b) SIM (`native_vsweep_parity.mm`,
  `run-sim-native-vsweep.sh`, booted sim, 16/16) vs OCCT `MakePipeShell`: straight morph
  vol/area/centroid/bbox rel ≈ 1e-14 (machine-exact), coupled morph×scale straight rel ≈ 2.0e-3 within
  the deflection band, curved-arc morph rel ≈ 4.4e-4, watertight; the broadened `native_vsweep_fuzz`
  certifies the coupled regime to ~1e-16 vs the exact Boole's-rule closed form (0 DISAGREED / 2 seeds).
  HONEST DECLINE (→ OCCT, measured): mismatched section vertex counts (ruled morph pairs k→k), a
  NON-PLANAR guided spine (genuine corrected-Frenet the RMF frame does not reproduce — **the sharpened
  next blocker: non-planar guided morph via the guide surface**), a coincident/collapsing guide, a
  self-folding morph. OpenSpec changes `moat-vsweep-variable-section` + `moat-vsfix-coupled-morph`.
  Bounded per family. ~0.5–1 py.
...
| **M6** completeness / fuzzing harness | test infra + `ssi/` | — | ✅ **Wave-1 + breadth×24 LANDED** — curved-boolean + STEP round-trip + construction loft/sweep + blend fillet/chamfer + wrap-emboss + mass-properties + **geometry-services (GS3 distance / GS4 curvature / GS2 section incl. OBLIQUE / GS5 inertia / GS6 validity / GS1 HLR vs OCCT, incl. tilted regimes; 0 DISAGREED across 2 seeds, 480 trials)** + **transform-chains (three-way native/OCCT/closed-form similarity; translate/rotate/uscale/mirror, N=160×2 seeds, 0 DISAGREED, mirror handedness-flip confirmed; found + gated an OCCT zero-scale hang)** + **reference/datum-geometry (axis/plane/edge-line/rim/offset/tangent datum queries on random solids at random rigid poses vs OCCT topology queries + closed-form datum image; N=96×2 seeds, 480 AGREED / 0 DISAGREED / 0 ORACLE_UNRELIABLE each)** + **direct-modeling (`cc_split_plane` / `cc_replace_face` cap offset / `cc_project_point_on_face` through the `cc_*` facade under BOTH engines vs a closed-form arbiter; N=80×2 seeds, 0 DISAGREED / 0 ORACLE_UNRELIABLE each; surfaced the OCCT half-space-cut grow limitation as ORACLE-INACCURATE, native vindicated)** + **transformed-boolean (the FIRST interaction fuzzer: composes a rigid transform WITH a boolean — random rigid-placed all-planar operands [box/ngon/concave-L] × FUSE/CUT/COMMON × translate/rotate/mirror through `cc_*` under BOTH engines; PRIMARY arbiter the rigid invariant `|T(A)∘T(B)|==|A∘B|`, exact for planar operands; N=96×2 seeds, 0 DISAGREED / 0 ORACLE_UNRELIABLE each; the volume invariant held to ≤1e-15 on every trial — native operand-Location folding into the planar BSP-CSG is EXACT)** + **orthographic-HLR / drafting (random solids [box/ngon/cylinder/cone-frustum/sphere/freeform] at random rigid poses from random view directions through `cc_hlr_project` under BOTH engines vs the OCCT HLRBRep oracle; visible/hidden 2D segment sets compared as a bidirectional labelled point-set with a closed-form cylinder/sphere silhouette-tangency arbiter; N=60×2 seeds, 103 AGREED / 17 HONESTLY-DECLINED / 0 DISAGREED / 0 ORACLE_UNRELIABLE; LOCALIZED a grazing-view length-proxy over-fire to a discretization artifact via bidirectional partition — native complete, no tolerance widened)** + **shape-healing (deliberately-defected soups — sew-jitter / flipped-face / seam-gap / short-edge / collinear-vertex / missing-face / beyond-tol — of a KNOWN base solid, native `heal::healShell` vs OCCT `ShapeFix`/`Sewing`, arbitrated by the undamaged solid's closed-form vol+area as a THIRD oracle; N=120×2 seeds, 0 DISAGREED / 0 ORACLE_UNRELIABLE each; the equal-or-more-conservative contract — native never emits a watertight solid ≠ truth, an honest decline is always safe — is the load-bearing invariant; closes the domain moat-m6h deferred)** + **curved-blend (the FIFTEENTH domain, moat-m6o — certifies the NEW M3 curved-blend native surface `src/native/blend/{curved_fillet,curved_shell,curved_offset,canal_fillet}.h`: random analytic-revolve base solids [capped cylinder / cone frustum / sphere-cap dome] driven through the SHIPPING `cc_fillet_edges` / `cc_shell` / `cc_offset_face` facade under BOTH engines vs OCCT [facade `BRepFilletAPI` / `MakeThickSolid` for fillet/shell; direct `BRepPrimAPI` for the planar-only offset facade] + a CLOSED-FORM removed/re-radius/wall volume as the PRIMARY arbiter; the nine families {fillet,shell,offset}×{cyl,cone,sphere} compared on volume/area/watertight/Euler χ=2/direction; N=72×2 seeds, 0 DISAGREED / 0 ORACLE_UNRELIABLE each; LOCALIZED — during bring-up — a spurious cone-offset DISAGREE to the HARNESS oracle omitting the perpendicular `d/cosσ` cap-radius shift [native was MORE correct, matching `curved_offset.h`], fixed in test-infra, not the product; closes the curved SHELL/OFFSET the 4th blend domain explicitly deferred)** + **draft-angle (the SIXTEENTH domain, moat-m6p — certifies the NEW native DRAFT-ANGLE surface `src/native/feature/draft_faces.h`: random prismatic solids [box / regular n-gon prism] × random drafted-side-face subsets [single + multi] drafted about the base plane by a random valid angle through the SHIPPING `cc_draft_faces` facade under BOTH engines vs OCCT `BRepOffsetAPI_DraftAngle` + a CLOSED-FORM drafted-volume arbiter [base footprint polygon clipped by each drafted edge's inward-shifted `z·tanθ` half-plane, integrated exactly over height — handles adjacent-face corners exactly]; four families {box,ngon}×{single,multi}-face compared on volume/area/watertight/Euler χ=2/strict-shrink; N=72×2 seeds, 0 DISAGREED / 0 ORACLE_UNRELIABLE each, every AGREE native==OCCT==closed-form to ~1e-16 [planar draft volume is EXACT]; the native arm HONESTLY-DECLINES a fraction of adjacent-corner box multi-face poses [ResolveFailed → OCCT] — first-class equal-or-more-conservative, confirmed by `volO==volX` in every declined case)** + **interference/clash (the SEVENTEENTH domain, moat-m6q — certifies the newly-native `cc_interference` surface `src/native/analysis/interference.h`, the OCCT-free header-only CLASH/TOUCHING/CLEAR classifier over the B3 membership + M0 mesh vocabulary: random PAIRS of solids [box / regular n-gon prism / faceted cylinder / UV-sphere] at random dims placed by a shared rigid transform [translate + rotate] spanning the three regimes — CLEAR / TOUCHING / CLASH — with a flush knife-edge jitter [exact-flush / slight-penetrate / slight-gap] stressing the coplanar-safe TOUCHING boundary and a non-watertight soup probe stressing the honest-decline precondition; native `meshInterference` vs OCCT [`BRepAlgoAPI_Common` volume + `BRepExtrema_DistShapeShape` distance] + a CLOSED-FORM arbiter where present [box∩box intersection-box volume + axis gap; sphere∩sphere lens volume + centre-distance regime]; four families × three regimes; N=72×2 seeds [+2 extra seeds at N=96], 62 AGREED / 10 HONESTLY-DECLINED / 0 DISAGREED each, complete per-cell coverage; a curved-pair TOUCH↔CLEAR facet straddle is classified FACET-CONVERGENT [deflection chord-error, native NOT at fault]; LOCALIZED — during bring-up — a genuine native limitation [`interference.h` step-4 min tri–tri distance uses the six vertex-vs-face sub-tests and omits the edge-edge term, so two boxes with EXACTLY coplanar faces overlapping in a plus-sign CROSS with no mutually-contained vertex mis-report a flush TOUCH as CLEAR — confirmed OCCT-free on the host gate, outside the certified seated/coincident/contained/slid assembly-mate contact envelope which all resolve correctly; REPORTED for a future product edge-edge fix, NOT changed in this test-infra track])** + **freeform-boolean (the EIGHTEENTH domain, moat-m6r — certifies the LARGE freeform-boolean surface that landed THIS campaign and the original curved-boolean fuzzer `native_boolean_fuzz.mm` predates: `src/native/boolean/{half_space_cut,slab_disjoint_cut,curved_wall_cut}.h` + the `ssi_boolean` Steinmetz. A seeded generator draws random VALID cut POSES per pose-family — off-centre freeform half-space at a random plane offset [CUT keep x≤c + COMMON keep x≥c], a disjoint slab at a random half-width [two-lump CUT], a dome/bowl curved-wall closed-circular-seam at a random cut height [CUT + COMMON], and the equal-R orthogonal bicylinder [Steinmetz COMMON] — and drives EACH through the SHIPPING native verb [`freeformHalfSpaceCut` / `freeformSlabDisjointCut` / `curvedWallHalfSpaceCut` / `nb::boolean_solid`] vs OCCT `BRepAlgoAPI_{Cut,Common,Fuse}` on a reconstruction of the SAME operand [Geom_BezierSurface prism / bowl-cup sewn to a solid] AND — the PRIMARY arbiter — a CLOSED-FORM volume [polynomial footprint integral `∫∫(H0+a(x²+y²))dA`; paraboloid-cap segment `π·ρ²·c/2`; Steinmetz `16R³/3`; partition identities]; five pose-families × op compared on volume/area/watertight/Euler χ=2. Two seeds (0xF4EE0FB001 N=72 → 74 AGREED / 15 HONESTLY-DECLINED / 0 DISAGREED / 0 ORACLE_UNRELIABLE; 0xBEEF5EED22 N=80 → 80 / 19 / 0 / 0): **0 DISAGREED** on both seeds, each pose-family with ≥1 AGREED, max native-vs-OCCT rel ≤1.5e-2 under a FIXED never-widened PER-FAMILY band [flat 2e-2 for the near-exact polynomial prism/slab/Steinmetz at deflection 0.004; curved-cup 3e-2 at the fine deflection 0.001 — >3× TIGHTER than the 0.10 the landed `native_curved_wall_cut_parity` validated for this exact cup]. LOCALIZED — during bring-up — that the curved-wall CUT native-vs-closed gap is a MONOTONE inscribed-facet MESH bias [d=8e-3→6.5% … 5e-4→0.55%; native B-rep exact, OCCT uses exact BRepGProp], and that the curved-wall COMMON identity `V(full)−V(cut)` is catastrophically cancellation-sensitive near the paraboloid rim [20% off at c=0.244 vs rim 0.245 while native==OCCT to ~3e-4] → the closed form is the PRIMARY arbiter only in the well-conditioned interior, near-rim probes cross-check the two engines; freeform FUSE is an honest domain-level decline. Determinism re-verified [same seed → byte-identical batch]. `src/native`/`src/engine`/`include`/`cc_*` ABI byte-unchanged; on run-sim-suite.sh SKIP list [own main(), OCCT+numsci slice, `std::_Exit`])** + **variable-section sweep (the moat-m6s domain — certifies the newly-native VARIABLE-SECTION / guide+spine sweep `src/native/construct/sweep.h` `build_variable_sweep` / `build_variable_sweep_tube`: random morph profiles [circle→circle radius-varying / regular-polygon→polygon / section-A→section-B per-vertex-radii] × random spine [straight +Z / smooth planar quarter-arc] × optional +X-splaying guide rail driven through the SHIPPING `cc_variable_sweep` facade under BOTH engines vs OCCT `BRepOffsetAPI_MakePipeShell` multi-section + a CLOSED-FORM volume arbiter [circle→circle straight = truncated cone `πH/3(r0²+r0r1+r1²)`; polygon / section-A→B / guided straight = exact prismatoid `∫₀¹ A(f)·H df` composite-Simpson, A(f) degree ≤4] compared on volume/area/watertight/Euler χ=2; five families {circle-morph / polygon-morph / section-A→B straight; guided straight; circle-morph curved}; N=72×2 seeds, 60 AGREED / 4 HONESTLY-DECLINED / 0 DISAGREED / 0 ORACLE_UNRELIABLE each, complete per-family AGREE coverage; honest-declines the out-of-envelope poses [mismatched vertex counts / coincident guide start → BOTH-DECLINED; non-planar guided helix → native NULL → OCCT] via a read-only `build_variable_sweep` decline probe; LOCALIZED — during bring-up, confirmed by first-principles math AND two agreeing oracles [OCCT 24-section densified pipe-shell + the exact polygon-clip Simpson integral agree to ~1e-4, native diverges 1–20% scaling with the guide splay] — a genuine native limitation [the STRAIGHT-spine guided builder used a two-station linear ruling that DROPPED a morph×scale cross-term when BOTH the section morphs AND the guide scale varies, needing ≥3 stations; native EXACT for {constant section + splaying guide} and {morph + spine-parallel guide}] — **RESOLVED by `moat-vsfix-coupled-morph`: the straight-spine guided builder now densifies to ≥3 stations bounded by the coupled area-integral (`straightCoupledStations`), so the coupled morph×scale case is NATIVE too; the divergence (19% → deflection-bounded match, converging with station count) is closed, the two exact sub-regimes stay byte-identical at 2 stations, and the fuzzer's guided family now certifies the coupled regime against the exact closed form (0 DISAGREED / 2 seeds) with NO tolerance widened**])** + **N-sided fill (the EIGHTEENTH domain, moat-m6t — certifies the newly-native bounded surface patch `src/native/surface/{ngon_fill,fill_solid}.h`, the OCCT-free Coons/Gregory transfinite interpolant of a 3–6-sided ANALYTIC boundary loop to a tessellated mesh patch: random loops for four families {planar-Ngon [general plane] / planar-hole-completion [coordinate-plane box-face restore] / saddle-4sided [bounded-warp non-planar] / arc-boundary [≥1 circular-arc side]} × N∈{3,4,5,6} driven through the SHIPPING `cc_fill_ngon` facade under BOTH engines vs OCCT `BRepFill_Filling` + a CLOSED-FORM exact-polygon-area arbiter where one exists [planar Newell area == exact fill; hole-completion == exact restored-face area] + an OCCT-independent analytic-boundary residual; compared on area/bbox-containment/boundary-coincidence — NOT interior-vertex identity, since OCCT's energy-minimizing patch differs from the transfinite one; N=72×2 seeds, 68 AGREED / 0 DISAGREED / 0 ORACLE_UNRELIABLE / 4 BOTH-DECLINED each, complete per-family coverage, every planar AGREE native==OCCT==closed-form to ~6 digits; a self-intersecting bowtie decline probe [N≥4] confirms the native NULL→OCCT scope bound; the saddle amplitude is bounded to the small-warp regime where the transfinite and energy-min surfaces co-area within the FIXED band [a generator scope bound, NOT a tolerance widening — an unbounded N=6 saddle at h≈0.6·R produces two different valid surfaces ~18% apart in area, removed by the bound])** + **sheet-metal (the moat-m6u domain — certifies the newly-native constant-thickness SHEET-METAL first slice `src/native/sheetmetal/{base_flange,edge_flange,unfold,common}.h` driven through the SHIPPING `cc_sheet_base_flange` / `cc_sheet_edge_flange` / `cc_sheet_unfold` facade under the NATIVE engine — the ONLY domain in the campaign with NO OCCT ORACLE, because OCCT core has NO sheet-metal module: the ARBITER is CLOSED FORM + a round-trip INVARIANT + closed-2-manifold validity, ground truth by construction. Random base profiles [rectangle / regular n-gon / convex-jittered n-gon] × thickness → base volume `|profileArea|·t` [EXACT, planar prism]; random `L×W×t` base flanged off its +X straight rim by a random bend [radius r / angle θ∈(0,180°) / wall height h] → folded volume `L·W·t + ½θ((r+t)²−r²)·W + h·t·W` [TRUE-cylinder bend meshed to deflection, converging FROM BELOW within the SAME 1.5% band `common.h::verifySolid` uses, NOT exceeding]; and the fold→unfold AREA INVARIANT at a random k-factor ∈[0,1] → developed area `baseArea + θ(r+k·t)·W + flangeArea` [the load-bearing round-trip, blank volume `devArea·t` EXACT] — compared on volume/area-invariant/watertight/oriented/χ=2. Two seeds (0x5EE7EA1F00, 0xB3ADF01DCC) N=72: **40 AGREED / 14 HONESTLY-DECLINED / 0 DISAGREED / 18 NATIVE-CHECK-INACCURATE each**, complete per-op coverage; base + unfold AGREE native==closed-form to ~1e-16, every folded part within the band, area-invariant residual ≤~5e-13. Out-of-slice poses [wrong/non-straight edge id / degenerate profile / angle outside (0,180°) / unfold of a non-fold body] HONEST-DECLINE to a native NULL. LOCALIZED a PRE-EXISTING product finding: `cc_check_solid`'s GS6 `no_self_intersection` sub-check FALSE-POSITIVES on the edge-flange bend [a fan of near-coplanar strips approximating a true cylinder; `first_failure`=5] while the part is watertight/oriented/χ=2/volume-exact — reproduces on the base commit [the landed `native_sheetmetal_selftest.mm` `edge_flange cc_check_solid valid` FAILs there too], so it is a GS6-vs-tessellated-cylinder interaction REPORTED for a future product fix, NOT a fold-geometry fault and NOT changed in this test-infra track → classified NATIVE-CHECK-INACCURATE [the no-OCCT-oracle analogue of ORACLE-INACCURATE], never a bar DISAGREE)** + **wrap-emboss freeform-base (the moat-m6v domain — closes the FREEFORM sphere-cap gap the cylinder-only `native_wrap_emboss_fuzz.mm` left: certifies the NEW F5 arm of `src/native/feature/wrap_emboss.h`, a RAISED circular pole boss on a FREEFORM sphere-cap dome [NON-developable base], alongside the cylinder base as a developable control, driven through the SHIPPING `cc_wrap_emboss` facade under BOTH engines. OCCT's own `cc_wrap_emboss` CANNOT wrap a non-cylindrical face — presented with a sphere wall it DECLINES [asserted 23/23 + 15/15 over the two seeds], so for the SPHERE base the CLOSED-FORM shell-sector delta `2π(1−cosφ0)·((R+h)³−R³)/3` is the SOLE arbiter, added to the base-dome volume OCCT DOES measure exactly [`cc_mass_properties`/BRepGProp]; the CYLINDER base uses the closed-form `A·|Rout²−R²|/(2R)` PRIMARY + OCCT [same facade, engine 0] SECONDARY vol+area cross-check; each in-scope native boss also checked watertight + Euler χ=2 + strictly-grown volume + mesh≈brep. Four base×mode cells {cylinder,sphere-cap}×{raised,recessed} — the sphere-recessed cell an honest DOMAIN-level decline [F5 is boss=1 only]. Two seeds (0x5745E6F00B N=64 → 47 AGREED / 3 HONESTLY-DECLINED / 0 DISAGREED / 0 ORACLE_UNRELIABLE; 0xB0551FEED2 N=64 → 41 / 5 / 0 / 0): **0 DISAGREED** on both seeds, each in-scope cell ≥1 AGREED, max native-vs-oracle bias cylVol=7.36e-3 sphVol=4.57e-3 under the FIXED never-widened tol; OpenSpec change `moat-m6v-wrap-emboss-fuzz`)** + **glTF round-trip (the moat-m6w domain — certifies the OCCT-FREE native glTF 2.0 writer `src/native/exchange/gltf_writer.{h,cpp}`, the `.gltf` JSON [base64 buffer] + `.glb` binary container the app hands to iOS RealityKit / QuickLook / SceneKit. A WRITER has NO OCCT equivalence to diff against [OCCT's `RWGltf` is a DIFFERENT implementation, not a spec oracle — two conforming glTF writers need not be structure-identical], so the ARBITER is ROUND-TRIP / SELF-CONSISTENCY computed independently in-test [no external deps, no reader in-repo]: (A) STRUCTURAL VALIDITY — a self-contained glTF-2.0 + `.glb` validator asserts glb magic/version/total-length + JSON+BIN chunk layout with 4-byte alignment, buffer/bufferView byteLength+byteOffset consistency [offsets 4-aligned + in range, view spans inside the buffer], accessor componentType [5125 UNSIGNED_INT indices / 5126 FLOAT pos+nrm] + count×element-size == bufferView byteLength, POSITION accessor min/max EXACTLY the componentwise fp32 extrema, every index in [0,vertexCount), unit normals, and NO NaN/Inf anywhere; (B) GEOMETRY FIDELITY ROUND-TRIP [the core DISAGREED check] — parse the emitted buffers DIRECTLY and compare vertex count / triangle count / connectivity [index-for-index] / bounding box [metres, fp32 tol] against an INDEPENDENT reimplementation of the writer's documented compaction contract [drop out-of-range/negative-index triangles, keep only referenced verts reindexed first-touch, weld by index reuse, scale mm→m×1e-3]; the `.gltf` base64 buffer and the `.glb` BIN chunk must decode byte-identical [single-source buffer]; determinism [same mesh+mode → byte-identical file]. Eleven mesh families {box / tetrahedron / multi-solid [2–4 boxes] / random triangle soup / triangle fan / single triangle / 0-triangle empty-geometry / out-of-range+negative indices / tiny [sub-micron] + huge [1e5–1e7 mm] coordinate scales / huge triangle count [grid quads]} stress accessor min/max, fp32 precision, buffer offsets/counts, and the compaction/skip paths. Seeded splitmix64→xoshiro256** keyed ONLY by `FUZZ_SEED`; N=240×2 decorrelated streams per run: **480 AGREED / 0 DISAGREED / 0 DECLINED / 0 ORACLE-INACCURATE / 0 NATIVE-CHECK-INACCURATE**, re-verified across 7 seeds. HOST-ONLY [`tests/native/test_native_gltf_fuzz.cpp` under ctest, calls the native writer directly — no engine, no numsci link]; NO SIM leg, because a WRITER has no meaningful OCCT diff [`RWGltf` is a distinct implementation, not a spec oracle] — the round-trip self-consistency arbiter is authoritative and complete on the host. An ORACLE-INACCURATE guard [distinct fp64 mm verts collapsing to one on-disk fp32 metre → the writer faithful to its documented f32 contract, the fp64 oracle over-precise] is present + justified but does not fire under these families [each vertex stays fp32-distinct even at extreme scale]. NOTE on the writer's header docstring: it says a null/empty mesh "writes NOTHING" — the raw `gltf_export_mesh` with a vertex-bearing 0-triangle input actually emits a fully-formed count:0 asset [byteLength:0 buffer, min/max=[0,0,0]], which is VALID glTF-2.0 and round-trips correctly [0 verts / 0 tris]; the no-op-write path is the FACADE's [`cc_gltf_export` tessellate declines a null body first], so this is a doc-vs-code nuance, NOT a native defect — no product code changed. OpenSpec change `moat-m6w-gltf-roundtrip-fuzz`)** + **display-mesh (the moat-m6w domain — certifies the render-quality DISPLAY mesh `src/native/render/display_mesh.h` [smooth per-vertex normals + crease-angle hard edges + optional UVs + optional edge-collapse LOD], a PURELY ADDITIVE post-process of the correctness tessellation, reached through the SHIPPING `cc_display_mesh` facade under BOTH engines. The post-process is engine-agnostic — the ONLY difference between the OCCT display mesh [engine 0, BRepMesh source] and the native display mesh [engine 1, SolidMesher source] is the SOURCE triangulation, and two meshers legitimately emit DIFFERENT triangle lists, so the harness does NOT diff triangle lists; the oracle is the INVARIANTS that must hold for BOTH regardless of the mesher [finite / unit-normal / non-degenerate / fold-watertight closed-2-manifold / UV∈[0,1] / LOD-reduces] PLUS the CLOSED-FORM analytic surface as the STRICTER arbiter on the primitive families [display-vertex distance to the exact sphere/cylinder/cone/box surface ≤ 6·deflection, Hausdorff budget 8·deflection under LOD — the load-bearing 'requested deflection actually respected' check] PLUS native-vs-OCCT bbox parity. Six families {sphere / cylinder / cone-frustum / box [analytic, closed-form-gated] + fillet-box [curved strip meeting planar faces] + boolean-cut [all-planar corner-notch BSP-CSG output]} × random scale × a broad deflection sweep [very tight 1e-3 → very loose 0.4·dim] × crease/LOD/UV toggles. Two seeds (0xD15B1A57EE, 0x0FF1CE9A11) N=72: **72 AGREED / 0 HONESTLY-DECLINED / 0 DISAGREED / 0 ORACLE-INACCURATE each**, complete per-family coverage; native holds the closed-form deflection bound to the fp floor [natSurf ~1e-15 — the native SolidMesher places every vertex exactly on the analytic surface] and bbox-parity with OCCT within the FIXED never-widened band, while native and OCCT tri counts differ up to ~2.4× [cone 30 vs 231; sphere 98,280 vs 201,194] — confirming the invariant oracle is correct where a triangle diff would false-fail. During bring-up the SCOPE GATE was made explicit: a native-engine UPSTREAM decline of the source body [the native boolean layer refuses a box∩cylinder curved cut → `cc_boolean`=0] is HONESTLY-DECLINED [out of the display-mesh SUT's scope, covered by native_boolean_fuzz], NOT a display-mesh DISAGREE; the boolean family was retargeted to an all-planar corner notch the native BSP-CSG builds exactly so the display mesh is genuinely exercised on a boolean output. Companion HOST-analytic arm `test_native_display_mesh_fuzz` [OCCT-free, closed-form surface distance over random sphere/cylinder/cone/box × varied deflection, 160 AGREED / 0 DISAGREED across 2 seeds]. `src/native`/`src/engine`/`include`/`cc_*` ABI byte-unchanged; on run-sim-suite.sh SKIP list [own main(), OCCT slice, `std::_Exit`])** fuzzers (0 DISAGREED, **24 native domains**); concave-stepped-shaft blends remain (gates M8) |
