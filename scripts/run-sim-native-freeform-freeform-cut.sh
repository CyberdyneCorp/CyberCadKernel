...
HARNESS="$REPO/tests/sim/native_freeform_freeform_cut_parity.mm"
...
echo "── compiling M2-freeform-freeform multi-face corner-clip weld native-vs-OCCT parity harness (iphonesimulator arm64)"
...
xcrun --sdk iphonesimulator clang++ -target arm64-apple-ios16.0-simulator -isysroot "$SYSROOT" \
  ...
  -o "$OUT/native_freeform_freeform_cut_parity"
...
xcrun simctl spawn "$UDID" "$OUT/native_freeform_freeform_cut_parity"
