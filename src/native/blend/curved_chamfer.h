...
// Assemble the CHAMFERED capped cylinder as a planar-facet soup. The only geometric
// difference from buildFilletedCylinder is that the wall→cap transition is a single
// CONE-FRUSTUM band (straight bevel) between the two setback circles, not a torus arc.
// Returns the polygons (empty on any degeneracy → NULL → OCCT). CCW-from-front winding
// per facet; assembleSolid welds.
inline std::vector<nb::Polygon> buildChamferedCylinder(const RimGeom& g, double d, double defl) {
  const double Rc = g.radius;
  const double Rin = Rc - d;                 // trimmed-cap radius (cap set back by d)
  if (!(Rin > 1e-9)) return {};              // Rc − d > 0 (else the bevel crosses the axis)
  ...
  const double hSeam = hCap - s * d;         // cylinder seam (wall set back axially by d)
  ...
  // 2. Frustum bevel band: ring(Rc, hSeam) → ring(Rin=Rc−d, hCap), N quads. ONE
  //    meridian step — the straight bevel. Outward = the 45° bisector radial + s·axis.
  for (int i = 0; i < N; ++i) {
    const double u0 = kTwoPi * i / N, u1 = kTwoPi * (i + 1) / N;
    const double um = 0.5 * (u0 + u1);
    const math::Vec3 radial = ax.x.vec() * std::cos(um) + ax.y.vec() * std::sin(um);
    const math::Vec3 outN = radial + ax.z.vec() * s;  // bevel normal (radial + s·axial)
    emitQuad(ringPoint(ax, Rc, u0, hSeam), ringPoint(ax, Rc, u1, hSeam),
             ringPoint(ax, Rin, u1, hCap), ringPoint(ax, Rin, u0, hCap), outN);
  }

  // 3. Trimmed cap: disk radius Rin (=Rc−d) at hCap, outward = capOut (toward +s end).
  {
