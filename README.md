...
- **Native core** (`src/native`) — OCCT-free C++20: `math` (vectors/transforms +
  ...
  `exchange` (native STEP AP203 EXPORT + IMPORT widened slice for in-scope native solids + foreign OCCT-written box/cylinder + multi-solid compounds + B-spline-face round-trip + ELLIPSE-curve recognition + rigid placed assemblies + AP203/AP214/AP242 headers), `ssi` (internal
  ...
  **de-risks (does NOT unblock/complete) curved blends #6 + wrap-emboss #7**, whose seams are
  ...
  ΔV 8.75e-04 = −0.088%; sphere FUSE ΔV ≤ 8.3e-04 / CUT ΔV ≤ 9.3e-04; all ΔV ≤ 9e-04, sim
  native-pass=12); Steinmetz FUSE/CUT still defer → OCCT;
...
| **blends: `cc_shell`** (uniform thickness, box-like planar solid — EXACT wall) | data exchange: **STEP IMPORT beyond the widened subset** (PMI, non-rigid/scaled/mirrored transforms, deep-nested assemblies, torus, ellipse-on-quadric solids, complex/trimmed profiles, rational B-splines) + **IGES export/import** (all IGES stays OCCT / dropped per the earlier decision) |
...
| **exchange: `cc_step_import`** (native OCCT-free Part-21 reader for the elementary/B-spline subset the writer emits + foreign OCCT-written box/cylinder — two-pass tokenizer/mapper, healed + self-verified watertight, host round-trip EXACT + sim OCCT parity `[NIMPORT]` 33/33 incl. foreign OCCT-written box/cylinder read natively at rel 0; **WIDENED**: multi-solid files → native `Compound` of watertight solids (rel 2.14e-16 vs OCCT), a native B-spline-FACE solid round-trips EXACT, the `ELLIPSE` curve entity is recognised/mapped, and a single-level **RIGID PLACED ASSEMBLY** (transform tree via CDSR → REPRESENTATION_RELATIONSHIP_WITH_TRANSFORMATION → ITEM_DEFINED_TRANSFORMATION AXIS2 pair) imports as a native PLACED `Compound` — 2-box verified vs OCCT: 2 solids, vol rel 3.74e-16, bbox Δ=0, faces 12/12; the reader is schema-independent so AP203/AP214/AP242 headers all import; any unsupported entity / torus / ellipse-on-quadric solid / non-rigid or scaled transform / Form-B `MAPPED_ITEM` / uncomposable transform tree / non-mm / non-watertight → NULL → OCCT `STEPControl_Reader`) | exchange: STEP import beyond the widened subset — PMI, non-rigid/scaled/mirrored transforms, deep-nested assemblies, torus, ellipse-on-quadric solids, complex/trimmed profiles, rational B-splines + all IGES import — declines to NULL, engine falls through to OCCT (honest, never fabricated) |
...
| **blends: `cc_fillet_edges`** (CONSTANT radius, convex planar-dihedral edge — rolling-ball cylinder, deflection-bounded; **+ CURVED circular rim, CONVEX *and* CONCAVE: a circular cylinder↔coaxial-planar rim → rolling-ball TORUS canal blend, G1-tangent at both seams, deflection-bounded, verified vs OCCT `BRepFilletAPI` — CONVEX cyl↔cap removes material (major `Rc−r`, `Rc≥2r`), CONCAVE boss↔larger-plane base rim ADDS material (major `Rc+r`, volume GROWS)**) | fillet: VARIABLE radius · cyl↔cyl / cyl↔cone canal · NON-circular crease (cone/sphere/ellipse/spline rim) · blind-hole bottom rim · freeform-neighbour · convex `Rc<2r` near-degenerate · seam-leaves-face · multi-edge (all → OCCT) |
...
bash scripts/run-sim-native-ssi-curved-boolean.sh # 18/18 — SSI S5-a/b/c/d curved booleans vs OCCT BRepAlgoAPI (native-pass=12: drill cyl∩cyl COMMON/FUSE/CUT + sphere∩sphere COMMON/FUSE/CUT (3/3) + branched Steinmetz COMMON/FUSE/CUT (op-set COMPLETE 3/3))
...
bash scripts/run-sim-native-step-import.sh   # 33/33 — STEP import (native slice, WIDENED) vs OCCT STEPControl_Reader (native box/cyl/holed-plate + FOREIGN box/cyl rel 0; multi-solid Compound rel 2.14e-16; B-spline-FACE round-trip exact; ELLIPSE recognised; rigid PLACED ASSEMBLY as native Compound rel 3.74e-16 bboxΔ=0; AP214 header accepted; torus + ellipse-on-quadric + non-rigid transform → OCCT)
```
