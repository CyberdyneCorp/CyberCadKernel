...
- **S4-e (two slices DONE)** — chart singularities (a surface's own `(u,v)` degeneracy on the
  locus). The **sphere parametric pole** (`v=±π/2`), the **cone apex**, AND the **FREEFORM
  NURBS collapsed-control-row pole** (a `uPeriod==0` genuine freeform degeneracy, crossed via the
  point-only `freeformChartInvert` far-side re-seed) are now DETECTED + CROSSED with the
  point-based fixed-plane cut (see the S4-e result section + capability row below). The **curve
  cusp** was DECLINED (by the IFT it coincides with the S4-c/S4-d pair-tangency regime → routed
  there/OCCT, no dead code). Still deferred: ASYMMETRIC/higher-order freeform poles whose
  continued-tangent re-seed does not verify, edge/seam degeneracies → OCCT.
...
## SSI-S4e result section (CHART SINGULARITIES — analytic sphere pole / cone apex + FREEFORM NURBS pole — SSI-ROADMAP S4-e, TWO SLICES; curve cusp declined by IFT)
...
### Second slice — FREEFORM NURBS parametric pole (SSI-ROADMAP S4-e-general, DONE)
The point-based corrector is normal-free, so it extends UNCHANGED to a genuine freeform pole: a
NURBS unit sphere built via `makeNurbsAdapter` (degU=degV=2, 9×5 poles, non-uniform knots,
rational weights, collapsed control ROWS at v=0 and v=2) carries `uPeriod == 0` — there is NO
analytic `u+π` meridian map. The ONLY new code is the far-side re-seed
`chartsing::freeformChartInvert` (a POINT-ONLY 1-D search for the far longitude at the fixed
near-pole latitude, using `S.point` alone — no `dU`, no near-zero normal); `chartFarUV` branches
on `uPeriod` and the analytic `uPeriod>0` path stays byte-identical. Host
`s4e_freeform_nurbs_pole_full_great_circle` LANDS (closed great circle, `singularitiesCrossed≥2`
= both freeform poles, `nearTangentGaps==0`, every node on both surfaces ≤1e-6); sim
`freeform-pole s4e singX=2 NTgaps=0 closed=1`, `len` native 6.2829 vs OCCT `GeomAPI_IntSS` 6.2832,
on both surfaces ≤1.51e-07 (chart OFF truncates at the first pole, half circle 3.1415). The DEFER
control `s4e_freeform_tip_endpoint_still_defers` (collapsed-row Bézier cone-tip on the v=1 DOMAIN
BOUNDARY — a genuine ENDPOINT, no far side) asserts `singularitiesCrossed==0`, `NearTangent`
(honest truncation → OCCT), no point fabricated past the tip. **Curve cusp — honest DECLINE:** a
cusp (marcher velocity `‖n_A×n_B‖→0`) is IDENTICALLY the S4-c/S4-d pair-tangency witness; by the
IFT a cusp with regular charts + healthy pair sine is the empty set, so a standalone S4-e cusp
witness would be unreachable dead code — none added; cusps route to S4-c/S4-d/OCCT. Living change
`openspec/changes/add-native-ssi-s4e-general` **archived** (`2026-07-07`).

### Honest scope — what S4-e does NOT do
The two elementary chart (removable) singularities — the **sphere parametric pole** and the **cone
apex** — plus the **FREEFORM NURBS collapsed-row pole** are crossed, each verified node-by-node on
both surfaces + on the OCCT locus. **ASYMMETRIC freeform poles** whose continued-tangent re-seed
does not verify on both surfaces, **edge/seam + higher-order degeneracies**, a degenerate-pole
B-spline SOLID fed to the marcher (no native construct builds one yet — fixtures are hand-seeded),
and **curve cusps** (declined by the IFT, routed to S4-c/S4-d) remain DEFERRED → OCCT, reported
with the measured gap, never faked. (**S4-f
...
| 5·SSI | `native-booleans` (SSI-ROADMAP **S5-a/b/c/d/e/f** curved boolean) | **slices done at the bar** | SSI-curve-driven split→classify→select→weld curved-boolean pipeline `src/native/boolean/ssi_boolean.{h,cpp}` (OCCT-free, `CYBERCAD_HAS_NUMSCI`-gated, consumes the S3 `TraceSet` — and, for S5-d, the S4-d branched re-trace). INTERNAL — invoked behind the existing `cc_boolean` op codes, no `cc_*` entry added; the native lib returns NULL when it cannot robustly handle an input and the ENGINE self-verify (watertight + correct-volume) falls back to OCCT. Produces **sixteen native curved-boolean sub-cases verified vs OCCT `BRepAlgoAPI_{Fuse,Cut,Common}`** (sim `run-sim-native-ssi-curved-boolean.sh` **24 passed, 0 failed, native-pass=18**): **S5-a** through-drill cyl∩cyl **COMMON** (unequal radii, transversal two-loop trace) — watertight, volN 3.1143 vs volO 3.1169, ΔV **8.14e-04**, ΔA 2.83e-04; **S5-b** through-drill cyl∩cyl **FUSE** (ΔV **8.80e-05**) + **CUT** (ΔV **4.03e-05**), assembler-only (fat wall with the two mouths cut out + planar-facet caps + reversed tunnel band / protruding end tubes, same shared `VertexPool` seams as S5-a); **S5-c** the sphere∩sphere op-set now **COMPLETE 3/3 native — COMMON + FUSE + CUT** (single closed seam; all three share the SAME decimated seam and reuse one generalised `appendSphereCap(outer,reversed)` cap builder + `VertexPool` weld, direction-slerp cap facets robust at the parametric pole): **COMMON** = two INNER (nearest-apex) caps (equal ΔV **4.13e-04** / unequal **4.66e-04**); **FUSE** (A∪B) = two OUTER (far-pole) caps, `V=V(A)+V(B)−lens` (equal ΔV **6.46e-04** volN 7.064 vs volO 7.0686 / unequal ΔV **8.34e-04** volN 8.1928 vs 8.1996); **CUT** (A−B, order-sensitive) = OUTER cap of A + INNER cap of B emitted REVERSED (inward normal bounds the scooped cavity), `V=V(A)−lens` (equal ΔV **6.98e-04** volN 2.8778 vs 2.8798 / unequal ΔV **9.29e-04** volN 6.0493 vs 6.0549) — all watertight, verified vs BOTH the analytic closed forms AND OCCT, no tolerance weakened; the COMMON path stays byte-identical (defaults `outer=false,reversed=false`); **S5-d** the **branched-trace Steinmetz bicylinder op-set now COMPLETE 3/3 native — COMMON + FUSE + CUT** (equal-R orthogonal cyl∩cyl): a `steinmetzPreGate` fires ONLY on the S4 decline edge (`nearTangentGaps>0`), RE-TRACES with `MarchOptions.enableBranchPoints=true`, `recogniseSteinmetzTrace` accepts only the canonical structure (`branchPoints==2`, four `BranchArc` arms), and the shared lune/arc split + `VertexPool` weld (`appendSteinmetzOuterWall` + the branched lune builder) drive all three ops from the SAME branched trace, differing only by which fragments survive + cap handling: **COMMON** `buildSteinmetzCommon` welds the four inside-the-other lune patches (keeps the four whose centroid is inside the other cylinder) into ONE watertight shell sharing the four arc seams + the two branch-point vertices (S5-a planar-facet + `VertexPool` weld) — verified vs **BOTH** the EXACT analytic `16 R³/3 = 5.33333` (host) **and** OCCT `BRepAlgoAPI_Common` 5.3333 (sim): volN **5.3287**, ΔV **8.75e-04** (−0.088%), ΔA **4.68e-04** (byte-identical to the S5-d baseline); **FUSE** `buildSteinmetzFuse` (A∪B) keeps the OUTSIDE wall regions of BOTH cylinders + all four original end caps, welded along the four arcs, `V=V(A)+V(B)−V(common)` — volN **32.385** vs OCCT `BRepAlgoAPI_Fuse` **32.366**, ΔV **5.82e-04**, ΔA **4.07e-03**; **CUT** `buildSteinmetzCut` (A−B) keeps A's OUTSIDE wall + A's caps + B's lune patches emitted REVERSED (inward normal, bounding the carved channel through A), `V=V(A)−V(common)` — volN **13.526** vs OCCT `BRepAlgoAPI_Cut` **13.516**, ΔV **7.22e-04**, ΔA **3.17e-03**; all watertight/closed/valid, inside the 1% curved-parity bar, no tolerance weakened. **S5-e** the **coaxial cone(frustum)∩cylinder op-set now COMPLETE 3/3 native — COMMON + FUSE + CUT** (the CONE surface family): a shared `coneCylSetup` prologue gates a coaxial cone+cylinder with a SINGLE closed S1-analytic seam circle (`r_c(s)=R0+s·tanα` equals `Rc`, apex-free, `nearTangentGaps==0`, no branch points) and resamples it to ONE pooled ring; all three ops split the two revolved walls at the seam and differ by fragment selection + cap handling: **COMMON** `buildConeCylCommon` welds the min-radius-profile solid of revolution (a frustum band inside the cylinder + a cylinder-segment band inside the cone) closed by two disc caps — volN **19.107** vs analytic **19.111355** vs OCCT `BRepAlgoAPI_Common` **19.111**, ΔV **2.03e-04**, ΔA **9.89e-05** (byte-identical to the S5-e COMMON baseline); **FUSE** `buildConeCylFuse` (A∪B) keeps both operands' OUTER wall regions (each band kept iff its mid-sample is strictly OUTSIDE the other) + the union's terminal disc caps + `appendAnnulusCap` step caps where an end-cap disc protrudes, `V=V(A)+V(B)−V(A∩B)` — volN **41.618** vs analytic **41.62610** vs OCCT `BRepAlgoAPI_Fuse` **41.626**, ΔV **2.04e-04**, ΔA **1.13e-04** (a GROW); **CUT** `buildConeCylCut` (A−B, cone minuend, order-sensitive) keeps A's OUTER wall + A's terminal/annular caps outside B + the cylinder's INSIDE-A band emitted REVERSED (inward radial, bounding the carved cavity, pinching to the pooled seam ring) + B's end-cap disc inside A reversed — a DISCONNECTED solid (detached cone tip + conical washer, one shell of two closed components), `V=V(A)−V(A∩B)` — volN **13.349** vs analytic **13.35177** vs OCCT `BRepAlgoAPI_Cut` **13.352**, ΔV **2.03e-04**, ΔA **1.02e-04** (a SHRINK); verified vs BOTH the analytic inclusion-exclusion closed form (engine `booleanResultVerified` with native `buildConeCylCommon` as `V(A∩B)`) AND OCCT, watertight/closed/valid, no tolerance weakened. **S5-f** the **coaxial cone(frustum)∩sphere op-set now COMPLETE 3/3 native — COMMON + FUSE + CUT** (the CONE∩SPHERE family): a shared `coneSphereSetup` prologue gates one `Cone` + one `Sphere` whose centre lies ON the cone axis on the FRUSTUM side, so `intersectSphereConeCoaxial` (a QUADRATIC in the cone parameter) yields EXACTLY ONE crossing `s*` strictly interior to both extents that does NOT cross the cone apex — a SINGLE closed S1-analytic circle seam (`nearTangentGaps==0`, `curveCount==1`) resampled to ONE pooled ring; the cone side reuses the S5-e cone-wall split (`appendRevolvedBand` + `appendDiskCap`), the sphere side the sphere-lens cap builder (`appendSphereCap`, inner/outer apex + reversed flags), the two poles CLASSIFIED against the cone into inner/outer: **COMMON** `buildConeSphereCommon` welds the min-cross-section overlap (the cone band inside the sphere + the cone terminal disc inside the sphere + the sphere INNER cap inside the cone), `V=V_frustum+V_spherical-segment` — volN **5.2546** vs OCCT `BRepAlgoAPI_Common` **5.2558**, ΔV **2.41e-04**, ΔA **1.28e-04**; **FUSE** `buildConeSphereFuse` (A∪B) keeps the sphere OUTER cap (outside the cone) + the cone OUTER wall band (outside the sphere) + the cone terminal disc bounding the union, `V=V(A)+V(B)−V(A∩B)` — volN **60.686** vs OCCT `BRepAlgoAPI_Fuse` **60.718**, ΔV **5.22e-04**, ΔA **2.61e-04** (a GROW); **CUT** `buildConeSphereCut` (A−B, cone minuend, order-sensitive) keeps A's OUTER wall + A's terminal disc(s) outside B + the sphere INNER cap emitted REVERSED (inward radial, the spherical dimple bounding the cavity, pinching to the pooled seam ring) — a CONNECTED frustum-with-spherical-dimple (ONE closed component), `V=V(A)−V(A∩B)` — volN **27.202** vs OCCT `BRepAlgoAPI_Cut` **27.207**, ΔV **1.96e-04**, ΔA **1.34e-04** (a SHRINK); verified vs BOTH the analytic inclusion-exclusion closed form (engine `ssiCurvedBooleanVerified` COMMON arm + generic `booleanResultVerified` for FUSE/CUT with native `buildConeSphereCommon` as `V(A∩B)`) AND OCCT, watertight/closed/valid, no tolerance weakened; the `sphere−cone` CUT (sphere minuend) declines → NULL → OCCT. Both gates green: host `test_native_ssi_curved_boolean` + `test_native_ssi_boolean` (S5-a COMMON + S5-b FUSE/CUT vs inclusion-exclusion closed form + S5-c equal/unequal lens vs analytic lens volume + **S5-d Steinmetz COMMON vs `16 R³/3` with 2-branch/4-arm assertions** + **the new `branched_fuse_cut_watertight_matches_analytic` case asserting FUSE=V(A)+V(B)−V(common) / CUT=V(A)−V(common), watertight, dispatcher non-null**, each ≤ 1% deflection band; honest NULL fixtures: tangent/disjoint spheres, thin−fat, **disjoint Steinmetz pair → NULL for all three ops** + **the sphere∩sphere FUSE/CUT analytic case (`FUSE=V(A)+V(B)−lens`, `CUT=V(A)−lens`, equal + unequal radii, watertight, CUT order-sensitivity `A−B≠B−A` asserted)** + **the coaxial cone∩cyl case extended with FUSE + CUT (`FUSE=V(A)+V(B)−V(A∩B)=41.62610` grows / `CUT=V(A)−V(A∩B)=13.35177` shrinks, both watertight, dispatcher + engine-self-verify non-null, `cyl−cone` order-sensitivity → NULL asserted)** + **the new `cone_sphere_coaxial_common_fuse_cut_watertight_matches_analytic` case (trace = 1 clean closed seam `nearTangentGaps==0`/`curveCount==1`; COMMON/FUSE/CUT all non-null + watertight + within 1% of the closed form `V(A∩B)=V_frustum+V_spherical-segment`, FUSE=V(A)+V(B)−V(A∩B) grows / CUT=V(A)−V(A∩B) shrinks; COMMON symmetric under operand swap; `sphere−cone` CUT → NULL asserted)**) — **NUMSCI OFF CTest 29/29** with the NUMSCI-gated SSI tests correctly ABSENT, **NUMSCI ON CTest 36/36**. No regression across tessellator/boolean/S5 suites; tessellator (`src/native/tessellate`) diff EMPTY (the S5-a lesson — every seam-adjacent face emits planar-triangle facets, mesher untouched); all prior native passes bit-identical (drill COMMON/FUSE/CUT, sphere∩sphere COMMON/FUSE/CUT, Steinmetz COMMON/FUSE/CUT, and the coaxial cone∩cyl COMMON/FUSE/CUT all persist; the final increment (native-pass 15→18) is exactly the coaxial cone∩sphere COMMON + FUSE + CUT). **Honestly DECLINING → OCCT (measured NULL fallbacks, never faked):** any branched pair that is NOT equal-R orthogonal Steinmetz (unequal-R / non-orthogonal / ≠ 2-branch / ≠ 4-arm), a disjoint Steinmetz pair (no seam), oblique / multi-tube cyl∩cyl, tangent/coincident pairs, transversal (non-coaxial) / apex-crossing cone pairs, cone∩cone, the sphere-minuend `sphere−cone` CUT, the TWO-circle coaxial cone∩sphere crossing (sphere through the cone / spanning the apex), apex-crossing / apex-in-extent frustums, TRANSVERSAL (non-coaxial) cone∩sphere (a quartic space curve), the reversed `cyl−cone` CUT, and other curved-curved families (transversal cyl∩cone, cyl∩sphere, sphere∩box, cone∩box, freeform). Living changes `add-native-ssi-curved-boolean` + `add-native-ssi-curved-boolean-wider` + `add-native-ssi-branched-boolean` + `complete-sphere-sphere-fuse-cut` + `complete-steinmetz-fuse-cut` + `add-native-cone-boolean` + `complete-cone-cyl-fuse-cut` + `add-native-cone-sphere-boolean` (all archived; branched `2026-07-05`, sphere fuse/cut + Steinmetz fuse/cut `2026-07-06`, cone∩cyl COMMON + FUSE/CUT + cone∩sphere COMMON/FUSE/CUT `2026-07-07`). See `openspec/SSI-ROADMAP.md` S5. |
...
| 5·SSI | `native-ssi` (SSI-ROADMAP **S4-e** CHART SINGULARITIES, FIRST SLICE) | **first slice done at the bar** | A **chart (removable) singularity** — where ONE surface's own `(u,v)` parametrization degenerates (`‖dU‖ → 0`) while its 3D point + normal stay finite: a **sphere parametric pole** (`v=±π/2`) or a **cone apex** (signed radius `=0`). The intersection can be perfectly TRANSVERSAL through it (pair sine need NOT collapse), yet S3's single-surface predictor `advanceParams` goes rank-1 there and truncates (spurious `BoundaryExit` on the sphere / node-budget step-crawl on the cone). Distinct from S4-c (surface *pair* grazes) and S4-d (*locus* self-crosses). Additive to `src/native/ssi/marching.{h,cpp}` + new OCCT-free `src/native/ssi/chart_singularity.h`, gated `CYBERCAD_HAS_NUMSCI`, default-**off** `enableChartSingularities`, no `cc_*` change, tessellator byte-identical, `src/native/**` OCCT-free. Four parts: **single-surface chart witness** (`chartConditionAt` finite-differences each surface's `‖dU‖` vs `‖dV‖·scale`; a collapse with finite normal flags THAT surface — NOT the pair sine, NOT a locus flip; a finite cap keeps a genuine boundary from being mistaken for a pole); **point-based fixed-plane-cut crossing** (`crossChartSingularity` makes bounded fine POINT-BASED jumps along the fixed last-good tangent `t★` — the branch_point.h/S4-c cut — which never touches the degenerate `dU`); **loose chart map-back** (sphere pole → opposite meridian `u+π`; cone apex → far nappe `v→−v`; the singular point itself never emitted); **honest guard** (a node is emitted ONLY if it verifies on BOTH surfaces ≤ `onSurfTol` with real along-`t★` progress; on ANY failure the whole band is DISCARDED (roll back) and the march STOPS + defers → OCCT as a `NearTangent` gap — no pole/apex point fabricated). Both gates green: host `test_native_ssi_s4e_singularities` **5 cases, 0 failed** (NUMSCI OFF CTest **26/26** S4-e TU ABSENT, NUMSCI ON CTest **32/32**) + sim `run-sim-native-ssi-marching.sh` **10 passed, 0 failed** — `sphere-pole s4e singX=2 NTgaps=0 closed=1`: a great circle crossing BOTH poles that S3 TRUNCATES at half loop (`len ≈ 3.1415`) is now FULLY traced (`len` native 6.2829 vs OCCT 6.2832, rel Δ 5.04e-05, on locus + both surfaces ≤ 1.51e-07); `cone-apex s4e singX=1 NTgaps=0 nodes=159 v1=[-2.00,2.00]`: a double-cone∩plane line through the apex that S3 STEP-COLLAPSES at is now FULLY traced across both nappes (159 nodes, on-locus 7.11e-16 / on-surface 6.79e-16). A genuine finite cylinder `v`-cap still exits as a `BoundaryExit` (chart machinery does NOT misfire); with the flag ON the S4-c graze still `crossed=22` and the S4-d Steinmetz still `branchPts=2 traced=4`. No regressions (`run-sim-suite.sh` 221/221, 5 transversal pairs `nt=0` bit-identical, S5 native-pass=6 persists). **Then extended by the SECOND slice (below):** the FREEFORM NURBS collapsed-row pole; the curve cusp declined by IFT. Living change `add-native-ssi-s4e-singularities` (archived `2026-07-05`). See the SSI-S4e result section + `openspec/SSI-ROADMAP.md`. |
| 5·SSI | `native-ssi` (SSI-ROADMAP **S4-e-general** FREEFORM parametric pole + curve-cusp assessment) | **second slice done at the bar** | Extends S4-e from the ANALYTIC pole to the FREEFORM analog: a NURBS unit sphere (via `makeNurbsAdapter`, degU=degV=2, 9×5 poles, non-uniform knots, rational weights, collapsed control ROWS at v=0/v=2) is a GENUINE freeform pole carrying `uPeriod == 0` — so the analytic `u+π` meridian map does NOT apply, and the degenerate normal `normalize(Sᵤ×Sᵥ)` is a finite near-zero `Dir3` (never NaN). Because the S4-c/S4-e point-based corrector is NORMAL-FREE it crosses this UNCHANGED; the ONLY new code is the far-side re-seed `chartsing::freeformChartInvert` — a POINT-ONLY 1-D search for the far longitude at the fixed near-pole latitude (`S.point` alone, no `dU`, no normal) — and `chartFarUV` branching on `uPeriod` (analytic `uPeriod>0` path byte-identical). Additive to `chart_singularity.h` + `marching.cpp`, gated `CYBERCAD_HAS_NUMSCI`, no `cc_*`/tessellator/tolerance change, `src/native/**` OCCT-free. Both gates green: host `test_native_ssi_s4e_singularities` **7 cases, 0 failed** (adds `s4e_freeform_nurbs_pole_full_great_circle` = LAND — closed great circle, `singularitiesCrossed≥2`, `nearTangentGaps==0`, every node on both surfaces ≤1e-6 — and `s4e_freeform_tip_endpoint_still_defers` = DEFER control — a collapsed-row Bézier cone-tip on the v=1 DOMAIN BOUNDARY, a genuine ENDPOINT with no far side, `singularitiesCrossed==0`, `NearTangent`, no point fabricated past the tip; NUMSCI OFF CTest **29/29** S4-e TU ABSENT, NUMSCI ON CTest **36/36**) + sim `run-sim-native-ssi-marching.sh` **11 passed, 0 failed** — `freeform-pole s4e singX=2 NTgaps=0 closed=1`: the NURBS-sphere ∩ plane that S3 truncated at the first pole (half circle 3.1415) now closes the full great circle (`len` native 6.2829 vs OCCT `GeomAPI_IntSS` 6.2832, on both surfaces ≤1.51e-07). No regressions (5 transversal pairs `nt=0` bit-identical, S4-c `crossed=22`, S4-d `branchPts=2`, analytic sphere-pole `singX=2` + cone-apex `singX=1` unchanged, S5 native-pass=18 persists). **CURVE CUSP — honest DECLINE (no dead code):** a cusp (marcher velocity `‖n_A×n_B‖→0`) is IDENTICALLY the S4-c/S4-d pair-tangency witness; by the IFT a cusp with regular charts + healthy pair sine is the empty set, so a standalone S4-e cusp witness would be unreachable — none added, cusps route to S4-c/S4-d/OCCT. **STILL DEFERRED → OCCT (honest):** ASYMMETRIC freeform poles whose continued-tangent re-seed does not verify, edge/seam + higher-order degeneracies, a degenerate-pole B-spline SOLID through the boolean pipeline (fixtures are hand-seeded, no native construct feeds one yet), S4-f self-intersection completeness. Living change `add-native-ssi-s4e-general` (archived `2026-07-07`). See the SSI-S4e result section + `openspec/SSI-ROADMAP.md`. |
