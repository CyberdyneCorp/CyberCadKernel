#!/usr/bin/env bash
# Compile the native-vs-OCCT AFFINE-TRANSFORM parity harness for the iOS simulator and run it
# inside a booted simulator via `xcrun simctl spawn`.
#
# This is Phase 4 capability #5 (`native-booleans`) verification gate 2 — the native-
# vs-OCCT parity pass (see openspec/NATIVE-REWRITE.md). #5 is the RESEARCH-GRADE
# capability: the native planar-polyhedron BSP-CSG boolean (src/native/boolean) is
# exercised through the cc_boolean facade under BOTH engines (cc_set_engine(0)=OCCT,
# cc_set_engine(1)=NativeEngine) and compared. Native intercepts axis-aligned box
# fuse/cut/common (EXACT), and everything outside the planar single-shell domain
# (curved operands, near-coincident, disjoint) FALLS BACK to OCCT (labelled, verified,
# never faked). Gate 1 (host unit tests, no OCCT) is built by CTest as
# test_native_engine with plain `clang++ -std=c++20`; this script does NOT need it.
#
# Like the sibling native_construct_parity harness, this drives the SHIPPING PATH: the
...
# always tests the current NativeEngine / native-boolean sources. No duplicate
...
HARNESS="$REPO/tests/sim/native_transform_parity.mm"
...
echo "── compiling native-boolean parity harness for iphonesimulator (arm64)"
...
xcrun simctl spawn "$UDID" "$OUT/native_transform_parity"
