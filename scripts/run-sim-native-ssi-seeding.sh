#!/usr/bin/env bash
# Compile the SSI Stage-S2 (subdivision seeding) native-vs-OCCT branch-RECALL harness
# for the iOS simulator, link it against the OCCT SSI oracle slice (GeomAPI_IntSS +
# Geom_* surfaces + point-projection Extrema) AND the NumPP/SciPP numsci archive (the
# least_squares refine), and run it inside a booted simulator via `xcrun simctl spawn`.
#
# This is the SSI-ROADMAP.md Stage-S2 verification gate 2 — the native-vs-OCCT recall
# pass. Gate 1 (host, no OCCT) is tests/native/test_native_ssi_seeding.cpp.
#
# Unlike the S1 harness (run-sim-native-ssi.sh, header-only + OCCT-free math), S2's
# refine needs the substrate, so this harness:
...
#   * links libnumsci_full_iossim_arm64.a (NumPP + SciPP-optimize/linalg) — build it
#     first with scripts/build-numsci.sh iossim (or reuse the eval/ archive);
...
HARNESS="$REPO/tests/sim/native_ssi_seeding_recall.mm"
...
echo "── compiling SSI-S2 seeding recall harness for iphonesimulator (arm64)"
...
xcrun --sdk iphonesimulator clang++ -target arm64-apple-ios16.0-simulator -isysroot "$SYSROOT" \
  ...
  -o "$OUT/native_ssi_seeding_recall"
...
xcrun simctl spawn "$UDID" "$OUT/native_ssi_seeding_recall"
