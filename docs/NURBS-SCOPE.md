...
| NURBS offset / thicken (robust) | 2–4 | 🟡 non-rational **surface offset + solid thicken/shell landed** (`src/native/math/bspline_offset` + `bspline_thicken`, Layer 5): offset = normal-locus sample + NURBS fit + adaptive refine, with a 2nd-fundamental-form self-intersection (fold) guard and honest error reporting; thicken = offset panel + original panel + four ruled walls sewn (shared exact boundary vertices) into a CLOSED watertight shell (`tessellate::Mesh`), closure proven (χ=2) + volume == `area·|d|` for a box + inherited fold guard; robust self-intersecting-shell trimming + multi-face shells + rational offset residual |
...
| Trimmed-NURBS B-rep data model + pcurve robustness + NURBS healing | 2–4 | 🟡 **partial** — trimmed-NURBS data model + point-in-region classification + pcurve fidelity/construction **landed** (`src/native/topology/trimmed_nurbs`, Layer 8); general tolerant-topology healing (gapped-loop closing, pinch-point resolution, near-coincident-pcurve snapping) + rational pcurve fitting residual |
