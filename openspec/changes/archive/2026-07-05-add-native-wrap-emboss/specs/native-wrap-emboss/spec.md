...
Native, OCCT-free wrap-emboss — **Feature #7 first slice** of `ROADMAP.md` /
`SSI-ROADMAP.md` (`S5 curved booleans → #7 wrap-emboss`). Emboss a RECTANGULAR pad
onto a CYLINDER lateral face behind the unchanged `cc_wrap_emboss` ABI, with the
Phase-3 OCCT `cc_wrap_emboss` as the oracle and the fallback.
