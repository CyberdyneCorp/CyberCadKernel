...
This change (Phase 4 #6) introduces the `native-blends` capability: it makes the **tractable
planar cases** of the fillet / chamfer / offset / shell feature-edit family NATIVE in
`NativeEngine` — **`cc_chamfer_edges`** (planar chamfer on a convex planar-planar edge),
**`cc_fillet_edges`** (constant-radius rolling-ball fillet on a convex planar-dihedral edge),
**`cc_offset_face`** (planar face offset), and **`cc_shell`** (uniform-thickness hollow of a
planar / box-like solid) — matching the OCCT oracle EXACTLY for chamfer / offset / box-shell
and within a deflection bound for the cylindrical fillet. A **mandatory self-verify guard**
(valid + watertight 2-manifold + a sane volume for the op) DISCARDS any bad native result and
falls through to OCCT. Curved-face inputs, concave edges, variable-radius
(`cc_fillet_edges_variable`), `cc_fillet_face`, multi-edge interference, and non-native bodies
fall through to OCCT (labelled, verified, never faked). No `cc_*` ABI change; default engine
stays OCCT.

> NOTE (honest scope): this is the **tractable-planar** slice of the blend family. A general
> robust fillet / offset / shell over arbitrary NURBS solids — blend surfaces tangent to
> curved faces, concave-edge trimming, variable-radius swept surfaces, face fillets, and
> multi-edge corner setback interference — is future work. It is fully acceptable that only
> chamfer + planar-dihedral constant fillet + planar offset (+ box shell) land native and
...
distance `radius / sin(halfAngle)`, tangent to both faces along two contact lines) — REUSING
the Phase-3 full-round tangent-cylinder construction in `src/native/construct` — trim both
...
deflection bound, with removed volume `(1 − π/4)·radius²·edgeLength`. This builder SHALL remain
