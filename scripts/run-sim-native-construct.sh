#!/usr/bin/env bash
# Compile the native-vs-OCCT CONSTRUCTION parity harness for the iOS simulator and
# run it inside a booted simulator via `xcrun simctl spawn`.
#
# This is Phase 4 capability #4 (`native-construction`) verification gate 2 — the
# native-vs-OCCT parity pass (see openspec/NATIVE-REWRITE.md). Gate 1 (host unit
# tests, no OCCT) is built by CTest as test_native_engine with plain
...
HARNESS="$REPO/tests/sim/native_construct_parity.mm"
...
echo "── compiling native-construction parity harness for iphonesimulator (arm64)"
...
xcrun simctl spawn "$UDID" "$OUT/native_construct_parity"
