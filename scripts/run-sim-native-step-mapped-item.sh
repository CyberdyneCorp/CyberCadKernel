#!/usr/bin/env bash
# Compile the native STEP Form-B assembly-INSTANCING parity harness (MOAT M4 import
# tail — MAPPED_ITEM / REPRESENTATION_MAP) for the iOS simulator and run it inside a
# booted simulator via `xcrun simctl spawn`. See
# openspec/changes/moat-m4t-assembly-import. It authors a genuine MAPPED_ITEM file (by
# splicing REPRESENTATION_MAP + MAPPED_ITEMs onto an OCCT-written box), reads it BOTH
# natively and through the OCCT STEPControl_Reader ORACLE, and asserts the placed
# instances match (count + per-solid vol/area/centroid/bbox/face+edge counts); a
# no-brep mapped rep is an honest native DECLINE that OCCT still imports.
#
# This is Phase 4 capability #7 (`native-data-exchange`, the STEP-IMPORT slice)
...
HARNESS="$REPO/tests/sim/native_step_mapped_item_parity.mm"
...
# (TKVCAF is deliberately OMITTED — it drags the full AIS presentation stack.)
# TKHLR provides HLRBRep_Algo / HLRBRep_HLRToShape — the HLR symbols the always-linked
# occt_drafting.cpp adapter references (this harness links the WHOLE kernel). Rest unchanged.
TKS="TKHLR TKDESTEP TKDEIGES TKXCAF TKCAF TKLCAF TKCDF TKBinL TKStdL TKXSBase TKDE \
...
xcrun --sdk iphonesimulator clang++ -target arm64-apple-ios14.0-simulator -isysroot "$SYSROOT" \
  ...
  -o "$OUT/native_step_mapped_item_parity"
...
xcrun simctl spawn "$UDID" "$OUT/native_step_mapped_item_parity"
