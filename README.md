...
| **internal SSI (S5-a): first SSI-curve-driven curved boolean** — the split→classify→select→weld pipeline (`ssi_boolean.{h,cpp}`, consumes the S3 `TraceSet`, guarded by `CYBERCAD_HAS_NUMSCI`) produces the **through-drill cylinder∩cylinder COMMON** (unequal radii, transversal two-loop trace): watertight, ΔV 8.1e-04, ΔA 2.8e-04 vs OCCT `BRepAlgoAPI_Common` (sim parity native-pass=1) | **SSI: wider S5 curved booleans** — fuse/cut (outside-fragment + re-trimmed-cap weld), other curved-curved families, and near-tangent pairs (incl. the equal-radius **Steinmetz** config, which is near-tangent → S4) decline to OCCT — honest NULL→OCCT fallbacks, never faked |
