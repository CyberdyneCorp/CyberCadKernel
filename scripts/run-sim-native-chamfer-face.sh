#!/usr/bin/env bash
# Compile the native-vs-OCCT BLEND parity harness for the iOS simulator and run it
...
HARNESS="$REPO/tests/sim/native_chamfer_face_parity.mm"
...
echo "── compiling native-blend parity harness for iphonesimulator (arm64)"
...
xcrun --sdk iphonesimulator clang++ -target arm64-apple-ios14.0-simulator -isysroot "$SYSROOT" \
  ...
  -o "$OUT/native_chamfer_face_parity"
...
xcrun simctl spawn "$UDID" "$OUT/native_chamfer_face_parity"
