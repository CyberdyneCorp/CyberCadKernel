#!/usr/bin/env bash
# Compile the native-vs-OCCT ANALYTIC-FACE-FILLET parity harness for the iOS simulator and run it
...
HARNESS="$REPO/tests/sim/native_analytic_fillet_parity.mm"
...
echo "── compiling native-analytic-fillet parity harness for iphonesimulator (arm64)"
...
xcrun --sdk iphonesimulator clang++ -target arm64-apple-ios14.0-simulator -isysroot "$SYSROOT" \
  ...
  -o "$OUT/native_analytic_fillet_parity"
...
xcrun simctl spawn "$UDID" "$OUT/native_analytic_fillet_parity"
