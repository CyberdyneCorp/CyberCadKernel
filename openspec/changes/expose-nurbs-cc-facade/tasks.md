...
- [x] 7.1 One mechanical-piece example per Wave D–I feature family, each rendering a
  ...
  handle. (**png**) — nine `examples/n*_*.py` scripts (also a revolved rational
  ring), each building via the `nurbs` API and rendering a magic-validated PNG
  into `examples/out/n*/`. All 9 render. (**png**)
- [x] 7.2 A gallery index (thumbnails + the feature each piece exercises) linked
  from `examples/README.md`; each script is a runnable, commented usage sample of
  the `nurbs` Python API. (**png**) — `examples/GALLERY.md` (thumbnail row + per-
  piece feature/API/geometry, honest single-surface-vs-face-set labelling),
  generated by `examples/run_all_nurbs.py`, linked from `examples/README.md`.

## 7b. BOOL-CC — general NURBS solid boolean (the landed L3 case, per §9.1)
- [x] 7b.1 `cc_nurbs_solid_boolean(wallA, rimA, lidA, wallB, rimB, lidB, op, deflection,
  out)` — build the two freeform bowl-cup SOLIDS (freeform Bézier wall + circular UV rim
  trim + flat lid, the minimal additional input documented in the header) from their wall
  `cc_surface`s, run the native LAYER-3 orchestrator `nurbsSolidBoolean` for
  fuse/cut/common, and fill a WATERTIGHT `CCMesh`. The MULTI-SEAM / non-watertight
  annulus↔annulus pose honest-declines (0 + `cc_last_error` + zeroed mesh) — NEVER a
  leaky mesh; no tolerance widened. Additive `src/facade/cc_nurbs_boolean.cpp` + the
  header's delimited J7 section; `src/native/**` untouched. (**host**, **abi**)
- [x] 7b.2 `nurbs.solid_boolean(...)` → `Mesh` (RAII / `KernelError`-from-`cc_last_error`):
  the canonical single-seam bowl-cup COMMON/CUT/FUSE → watertight `trimesh` mesh (χ=2) at
  the closed-form op-volume within the tessellation band; a multi-seam pose RAISES
  `KernelError`. `_cffi`/`_ffi` tables extended in lockstep + `test_ffi` symbol-count
  guard bumped (58 → 59 NURBS additions; 169 → 170 total). (**py**, **abi**)
...
- [x] 8.3 `openspec validate expose-nurbs-cc-facade --strict` green; `test_abi`
