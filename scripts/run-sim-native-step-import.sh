#!/usr/bin/env bash
# Compile the native STEP-IMPORT parity + foreign-STEP harness for the iOS
...
HARNESS="$REPO/tests/sim/native_step_import_parity.mm"
...
xcrun --sdk iphonesimulator clang++ -target arm64-apple-ios14.0-simulator -isysroot "$SYSROOT" \
  ...
  -o "$OUT/native_step_import_parity"
...
xcrun simctl spawn "$UDID" "$OUT/native_step_import_parity"
