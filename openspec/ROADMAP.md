...
- ◐ Primitive & swept-solid construction (extrude/revolve/loft/sweep). Change
  ...
  *(CORE done at the verification bar — the first ENGINE-WIRED capability. Native
  ...
  a straight or smooth-planar spine), and Tier D `cc_tapered_shank` (silhouette revolved
  360° about Z) now DONE at the bar; still OCCT-fallthrough (not faked): non-planar /
  tight-curvature / real-twist / guided / rail sweep, `cc_helical_thread` /
  `cc_tapered_thread` (native radial-V tiling built but self-verify defers to OCCT
  `MakePipeShell`), wrap-emboss, 3+-section / guided / rail loft, arc/spline revolve.
...
| 4b | `add-native-threads` (Tier D) | native-construction (advanced) | ◐ **Tier D shank done at verification bar; threads OCCT-fallthrough** — `cc_tapered_shank` (pointed-shank silhouette cone tip → full-radius cylinder → head disk revolved 360° about WORLD Z by reusing native `build_revolution`; TRUE on-axis apex, robustly watertight; matches OCCT `BRepPrimAPI_MakeRevol`) NATIVE (`src/native/construct/thread.h`, cognitive complexity ≤ 5). `cc_helical_thread` / `cc_tapered_thread` build the full radial-V axis-aux-spine helical tiling (correct volume + V geometry) but per-turn seams don't weld robustly watertight on the current mesher, so the engine `robustlyWatertight` self-verify REJECTS and FALLS THROUGH to OCCT `MakePipeShell` (labelled, verified, never faked). Host `test_native_thread` (8 cases) + `test_native_engine` (3 facade cases) CTest **16/16** no-OCCT (one TEST-ONLY fix — stale in-plane-Y bbox/axis assertions corrected to shipped Z-axial convention, impl unchanged) + iOS-sim native-vs-OCCT parity `native_thread_parity.mm` — `cc_tapered_shank` NATIVE r5/fh20/th10 vol o=1837.94 n=1830.27 rel 4.17e-03 / area rel 3.64e-03 / centroidΔ 3.85e-02 / bbox maxCornerΔ 1.00e-07 / F 4→9 (k=3 tiling) / watertight 144 tris, plus 2 thread ops OCCT fall-through (native active=1, self-verify defers, vol rel 0.00e+00). No regressions (`test_native_tessellate` green, `run-sim-suite.sh` 221/221). |
| 4b | `add-native-swept-solids` (Tier E) | native-construction (advanced) | ☐ follow-up — E wrap-emboss (+ 3+-section / guided / rail loft, non-planar / guided / rail sweep, `cc_helical_thread` / `cc_tapered_thread` robust-watertight thread mesh), currently OCCT-fallthrough. **PARTIALLY UNBLOCKED by #5:** wrap-emboss's add/subtract step is a boolean, so a PLANAR-polyhedron emboss/deboss can now use the native BSP-CSG booleans (fuse/cut) instead of OCCT; curved-surface wrap-emboss still needs curved native booleans (#5 curved slice) + native offset (#6). |
