#!/usr/bin/env bash
# Compile the native-vs-OCCT THREADS / TAPERED-SHANK parity harness (Tier D, Phase 4
# #4b) for the iOS simulator and run it inside a booted simulator via
# `xcrun simctl spawn`.
#
# This is the thread sibling of run-sim-native-sweep.sh (Tier C). It is capability
# #4b (`native-construction`, Tier D — threads / tapered shank) verification gate 2 — the
# native-vs-OCCT parity pass (see openspec/NATIVE-REWRITE.md) for cc_tapered_shank
# (native, a silhouette revolved 360° about Z) and the ATTEMPTED cc_helical_thread /
# cc_tapered_thread (verified OCCT fall-through today — the radial-V helical tiling
# self-verify defers). Gate 1 (host unit tests, no OCCT) is built by CTest as
# test_native_thread + test_native_engine with plain `clang++ -std=c++20`; this script
# does NOT need it.
#
...
# adapter reaches booleans, meshing, MakePipeShell/MakeRevol, STEP/IGES, etc.).
#
...
# always tests the current thread.h / NativeEngine / cc_set_engine sources. No
...
HARNESS="$REPO/tests/sim/native_thread_parity.mm"
...
# facade (construct/prism+revol+pipe-shell, booleans, meshing, mass/bbox queries, and —
# since occt_exchange.cpp is compiled in — STEP/IGES), so link the SAME broad toolkit
# set run-sim-suite.sh uses (most-derived → base).
...
echo "── compiling native-thread parity harness for iphonesimulator (arm64)"
...
xcrun simctl spawn "$UDID" "$OUT/native_thread_parity"
