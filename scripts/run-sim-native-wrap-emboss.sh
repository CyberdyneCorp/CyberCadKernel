#!/usr/bin/env bash
# Compile the native-vs-OCCT WRAP-EMBOSS parity harness for the iOS simulator and run it
# inside a booted simulator via `xcrun simctl spawn`.
#
# This is Phase 4 capability #7 (`native-wrap-emboss`) verification gate 2 — the native-
# vs-OCCT parity pass (see openspec/NATIVE-REWRITE.md). The native wrap-emboss slice
# (src/native/feature/wrap_emboss.h: a RECTANGULAR pad embossed on a CYLINDER lateral
# face, boss=1) is exercised through the cc_wrap_emboss facade under BOTH engines
# (cc_set_engine(0)=OCCT, cc_set_engine(1)=NativeEngine) and compared. Native intercepts
# the tractable rectangular-pad-on-cylinder case (DEFLECTION-BOUNDED faceting, welded
# watertight), and everything outside the domain (deboss, non-rectangular / non-
# cylindrical / off-end footprint, self-verify failures) FALLS BACK to OCCT (labelled,
# verified, never faked). Gate 1 (host unit tests, no OCCT) is built by CTest as
# test_native_wrap_emboss with plain `clang++ -std=c++20`; this script does NOT need it.
#
# Like the sibling native_curved_fillet_parity harness, this drives the SHIPPING PATH: the
...
# CYBERCAD_HAS_OCCT) + src/native/** — plus the full OCCT toolkit set (the OCCT adapter's
# wrap_emboss reaches BRepBuilderAPI_Sewing / ShapeFix / BRepAlgoAPI_Fuse, meshing,
# mass/bbox queries, etc.).
#
...
# always tests the current NativeEngine / native wrap-emboss sources. No duplicate
...
HARNESS="$REPO/tests/sim/native_wrap_emboss_parity.mm"
...
echo "── compiling native-wrap-emboss parity harness for iphonesimulator (arm64)"
...
xcrun --sdk iphonesimulator clang++ -target arm64-apple-ios14.0-simulator -isysroot "$SYSROOT" \
  ...
  -o "$OUT/native_wrap_emboss_parity"
...
xcrun simctl spawn "$UDID" "$OUT/native_wrap_emboss_parity"
