...
flowchart TD
    ...
    Engine -->|"cc_set_engine(1)"| Native["NativeEngine (C++20)<br/>native: math · topology · tessellation ·<br/>construction (extrude / revolve / spline extrude / torus revolve / 2- &amp; N-section loft / straight+planar+RMF sweep / tapered-shank / helical+tapered thread) ·<br/>booleans (planar-polyhedron + axis-aligned box-cylinder fuse/cut/common) ·<br/>blends (planar-dihedral fillet/chamfer/offset/shell + constant-radius CURVED fillet on a circular cylinder↔cap rim — rolling-ball TORUS canal, G1-tangent) ·<br/>features (wrap-emboss: rectangular pad on a cylinder lateral face, wrapped side walls + outer cap) ·<br/>analytic SSI S1 (elementary-pair intersection curves) + S2 seeding + S3 marching (transversal intersection curves / WLines traced for freeform/skew-quadric pairs, internal) + S4-a/b (coincident-region + tangent-contact classification) + S4-c (first near-tangent march-through) + S4-d (first branch-point slice: Steinmetz self-crossing localized + routed) + S4-e (first chart-singularity slice: sphere parametric pole + cone apex crossed) + S4-f (robust true-return loop-closure + self-intersection guard + adaptive completeness-critic re-seed — measured recall floor, not a proof) + S5-a/b/c/d (SSI-driven curved booleans: through-drill cyl∩cyl COMMON/FUSE/CUT + sphere∩sphere COMMON/FUSE/CUT (3/3) + branched Steinmetz COMMON/FUSE/CUT (3/3)) ·<br/>STEP export (in-scope native solids) ·<br/>STEP import (native slice, WIDENED: OCCT-free Part-21 reader for the AP203 elementary/B-spline subset + foreign OCCT-written box/cylinder + multi-solid Compound + B-spline-FACE round-trip + ELLIPSE-curve recognition, healed + self-verified watertight else → OCCT) ·<br/>shape healing FIRST SLICE (internal: tolerant sew + vertex/tolerance unification + degenerate removal + orientation fix of a coincident-within-tolerance face soup → watertight solid)"]
    Engine -.->|"no-OCCT host build"| Stub["Stub engine"]

    Native -.->|"fallthrough (still OCCT, all SSI/Tier-4):<br/>general curved booleans · CONCAVE/variable/non-circular-crease/cyl↔cyl-canal blends · deboss + non-rectangular/non-cylindrical/&gt;2π wrap-emboss ·<br/>STEP import beyond the widened subset (torus · ellipse-on-quadric solids · nested/transformed assemblies · AP242 · complex/trimmed profiles · rational B-splines) · IGES export/import · fine-pitch (self-intersecting) thread ·<br/>self-intersecting/tight/real-twist/guided/rail sweep · mismatched/guided/hard-rail loft · spindle torus · healing RESIDUAL (beyond-tol gaps · missing pcurves · self-intersecting wires · arbitrary broken B-rep)"| OCCT
...
- **Native core** (`src/native`) — OCCT-free C++20: `math` (vectors/transforms +
  ...
  `exchange` (native STEP AP203 EXPORT + IMPORT widened slice for in-scope native solids + foreign OCCT-written box/cylinder + multi-solid compounds + B-spline-face round-trip + ELLIPSE-curve recognition), `ssi` (internal
  ...
  plus **S4-e** — the FIRST CHART-SINGULARITY slice: where ONE surface's own `(u,v)` parametrization
  ...
  on-surface ≤ 6.79e-16), while a genuine finite cylinder `v`-cap still exits cleanly and any
  pole/apex that will not verify STILL defers → OCCT (no crossing fabricated);
  ...
  **de-risks (does NOT unblock/complete) curved blends #6 + wrap-emboss #7**, whose seams are
  ...
  the **branched-trace Steinmetz bicylinder op-set now COMPLETE 3/3 native — COMMON + FUSE + CUT**
  (equal-R orthogonal cyl∩cyl — split each cylinder along its arcs into inside-the-other lune
  patches; COMMON welds the four lunes, FUSE keeps both OUTSIDE walls + all four caps
  `V=V(A)+V(B)−V(common)`, CUT keeps A's OUTSIDE wall + A's caps + B's lunes REVERSED
  `V=V(A)−V(common)`; shared arc seams + branch-point vertices), verified watertight vs
  OCCT `BRepAlgoAPI_{Fuse,Cut,Common}` **and** the analytic closed forms (Steinmetz COMMON volN 5.3287
  ΔV 8.75e-04 = −0.088%, FUSE volN 32.385 vs OCCT 32.366 ΔV 5.82e-04, CUT volN 13.526 vs OCCT 13.516
  ΔV 7.22e-04; sphere FUSE ΔV ≤ 8.3e-04 / CUT ΔV ≤ 9.3e-04; all ΔV ≤ 9e-04, sim
  native-pass=12);
  ...
  AND the equal-R orthogonal Steinmetz op-sets are now COMPLETE 3/3 native), `numerics`
...
| **blends: `cc_shell`** (uniform thickness, box-like planar solid — EXACT wall) | data exchange: **STEP IMPORT beyond the widened subset** (torus, ellipse-on-quadric solids, nested/transformed assemblies, AP242/PMI, complex/trimmed profiles, rational B-splines) + **IGES export/import** (all IGES stays OCCT / dropped per the earlier decision) |
...
| **exchange: `cc_step_import`** (native OCCT-free Part-21 reader for the AP203 elementary/B-spline subset the writer emits + foreign OCCT-written box/cylinder — two-pass tokenizer/mapper, healed + self-verified watertight, host round-trip 9/9 EXACT + sim OCCT parity `[NIMPORT]` 28/28 incl. foreign OCCT-written box/cylinder read natively at rel 0; **WIDENED**: multi-solid files → native `Compound` of watertight solids (rel 2.14e-16 vs OCCT), a native B-spline-FACE solid round-trips EXACT, and the `ELLIPSE` curve entity is recognised/mapped; any unsupported entity / torus / ellipse-on-quadric solid / transformed assembly / non-mm / non-watertight → NULL → OCCT `STEPControl_Reader`) | exchange: STEP import beyond the widened subset — torus, ellipse-on-quadric solids, nested/transformed assemblies, arbitrary/AP242/PMI STEP + all IGES import — declines to NULL, engine falls through to OCCT (honest, never fabricated) |
...
| **blends: `cc_fillet_edges`** (CONSTANT radius, convex planar-dihedral edge — rolling-ball cylinder, deflection-bounded; **+ CURVED first slice: a convex circular cylinder↔coaxial-planar-cap rim → rolling-ball TORUS canal blend, G1-tangent at both seams, `Rc≥2r`, deflection-bounded, verified vs OCCT `BRepFilletAPI`**) | fillet: CONCAVE circular rim · VARIABLE radius · cyl↔cyl / cyl↔cone canal · NON-circular crease (cone/sphere/ellipse/spline rim) · freeform-neighbour · `Rc<2r` near-degenerate · multi-edge (all → OCCT) |
...
| construction: 2-section AND N-section (3+) ruled loft (equal-count planar sections) | — |
| **features: `cc_wrap_emboss`** (FIRST NATIVE SLICE: emboss `boss=1` a RECTANGULAR pad onto a CYLINDER lateral face — footprint wrapped `u=px/R, v=py+vMid`, wrapped side walls + circumferential walls + outer cap at `R+height`, welded watertight, verified vs OCCT `cc_wrap_emboss`) | wrap-emboss: DEBOSS (`boss=0`) · NON-rectangular / >4-corner / dense / high-curvature profile · NON-cylindrical (cone/sphere/planar/NURBS) base · footprint wrapping >2π / self-overlapping / off the axial ends (all → OCCT) |
...
| **internal SSI (S5-a/b/c/d): SSI-curve-driven curved booleans** — the split→classify→select→weld pipeline (`ssi_boolean.{h,cpp}`, consumes the S3 `TraceSet` — and, for S5-d, the S4-d branched re-trace; guarded by `CYBERCAD_HAS_NUMSCI`) produces the **through-drill cylinder∩cylinder COMMON/FUSE/CUT** (unequal radii, transversal two-loop trace), the **sphere∩sphere COMMON / FUSE / CUT** op-set now COMPLETE 3/3 native (single closed seam; one generalised `appendSphereCap(outer,reversed)` cap builder + `VertexPool` weld on the SAME decimated seam — COMMON = two INNER caps; FUSE (A∪B) = two OUTER far-pole caps, `V=V(A)+V(B)−lens`; CUT (A−B, order-sensitive) = OUTER cap of A + INNER cap of B emitted REVERSED, `V=V(A)−lens`; direction-slerp facets robust at the parametric pole), and the **branched-trace Steinmetz bicylinder COMMON / FUSE / CUT** op-set now COMPLETE 3/3 native (equal-R orthogonal cyl∩cyl — a `steinmetzPreGate` + branch-enabled re-trace + `recogniseSteinmetzTrace` for the canonical 2-branch-point / 4-`BranchArc` structure drive the shared lune/arc split + `VertexPool` weld: `buildSteinmetzCommon` welds the four inside-the-other lunes; `buildSteinmetzFuse` keeps both cylinders' OUTSIDE walls + all four caps `V=V(A)+V(B)−V(common)`; `buildSteinmetzCut` keeps A's OUTSIDE wall + A's caps + B's lunes REVERSED `V=V(A)−V(common)`): all watertight, ΔV ≤ 9e-04, ΔA ≤ 5e-04 vs OCCT `BRepAlgoAPI_{Fuse,Cut,Common}` — additionally verified vs the EXACT analytic inclusion-exclusion volumes (COMMON `16 R³/3 = 5.33333` volN 5.3287 ΔV 8.75e-04 = −0.088%, FUSE volN 32.385 vs OCCT 32.366 ΔV 5.82e-04, CUT volN 13.526 vs OCCT 13.516 ΔV 7.22e-04), and the sphere FUSE/CUT vs the analytic closed forms (FUSE ΔV ≤ 8.3e-04, CUT ΔV ≤ 9.3e-04) (sim parity native-pass=12) | **SSI: wider S5 curved booleans** — any branched pair that is NOT equal-R orthogonal Steinmetz (unequal-R / non-orthogonal / ≠ 2-branch / ≠ 4-arm), other curved-curved families (cyl∩cone, cyl∩sphere, cone∩cone, sphere∩box, cone∩box, freeform), and near-tangent / coincident / concentric / disjoint pairs decline to OCCT — honest NULL→OCCT fallbacks, never faked (sphere∩sphere AND equal-R orthogonal Steinmetz fuse/cut/common are now NATIVE — see the native column) |
...
bash scripts/run-sim-native-ssi-curved-boolean.sh # 18/18 — SSI S5-a/b/c/d curved booleans vs OCCT BRepAlgoAPI (native-pass=12: drill cyl∩cyl COMMON/FUSE/CUT + sphere∩sphere COMMON/FUSE/CUT (3/3) + branched Steinmetz COMMON/FUSE/CUT (3/3))
...
bash scripts/run-sim-native-step-import.sh   # 28/28 — STEP import (native slice, WIDENED) vs OCCT STEPControl_Reader (native box/cyl/holed-plate + FOREIGN box/cyl rel 0; multi-solid Compound rel 2.14e-16; B-spline-FACE round-trip exact; ELLIPSE recognised; torus + ellipse-on-quadric → OCCT)
```
...
| **4 — Native rewrite** | replace OCCT capability-by-capability, then drop it | ◐ **substantially native (planar/analytic), progressing on the curved tail** — native math · topology · tessellation · construction (incl. spline/torus/N-loft/RMF-sweep/threads) · planar+box∩cyl booleans · planar blends · STEP export; **numeric foundations adopted (NumPP + SciPP)**; **SSI S1** (analytic intersection) + **S2** (subdivision seeding) + **S3** (marching-line tracer — full transversal intersection curves / WLines) + **S4-a/b** (coincident-region + tangent-contact classification) + **S4-c** (first near-tangent march-through — crossable `NearTangentTransversal` graze now traced) + **S4-d** (first branch-point slice — the **Steinmetz** self-crossing bicylinder localized + routed through both branch points; isolated tangent point still ends) + **S4-e** (first chart-singularity slice — a marched curve crossing a **sphere parametric pole** / **cone apex** now fully traced vs OCCT `GeomAPI_IntSS`, sphere great circle `singX=2 len 6.2829 vs 6.2832`, cone apex `singX=1` both nappes) + **S5-a/b/c/d** (SSI-driven curved booleans: through-drill cyl∩cyl COMMON/FUSE/CUT + sphere∩sphere COMMON/FUSE/CUT op-set COMPLETE 3/3 + branched Steinmetz bicylinder COMMON/FUSE/CUT op-set COMPLETE 3/3, verified vs OCCT + the analytic closed forms incl. the exact `16 R³/3`, native-pass=12) done vs OCCT; **first CURVED-tail feature slices — #6 curved fillet** (convex circular cylinder↔cap rim → rolling-ball TORUS canal, G1-tangent, verified vs OCCT `BRepFilletAPI`) **and #7 wrap-emboss** (rectangular pad on a cylinder lateral face, watertight, verified vs OCCT `cc_wrap_emboss`) **now native**; **STEP import FIRST SLICE** (OCCT-free Part-21 reader for the AP203 elementary/B-spline subset + foreign OCCT-written box/cylinder, healed + self-verified, host round-trip 9/9 EXACT + sim `[NIMPORT]` 15/15 vs OCCT `STEPControl_Reader`) **now native**. Pending: SSI **S4-d general/freeform + S4-e general/freeform + S4-f** marching core (general/freeform branch points, higher-order/curve cusps, general/freeform parametric singularities, self-intersection completeness, deeper near-coincident bands — the moat tail) → wider S5 curved booleans (general non-Steinmetz branched pairs, more families — the equal-R orthogonal Steinmetz op-set is now COMPLETE 3/3), general curved blends (concave / variable / non-circular-crease / cyl↔cyl-canal fillets), general wrap-emboss (deboss / non-rectangular / non-cylindrical base), STEP import beyond the AP203 elementary/B-spline subset (assemblies / AP242 / complex profiles → OCCT; the first native import slice + foreign OCCT-written box/cylinder now land natively, host round-trip 9/9 + sim `[NIMPORT]` 15/15), all IGES import/export (stays OCCT), shape-healing residual. drop-occt (#8) BLOCKED on these (research-grade, ≈9–18 py). See [openspec/SSI-ROADMAP.md](openspec/SSI-ROADMAP.md). |
