...
| offset_face | cc_offset_face | 1321 | **A** | curved cyl-wall done M3co (resid cone/sphere-wall) | 10 | planar face native (1322); **CYLINDER LATERAL WALL radial offset native** (`curved_offset.h`: capped-cyl re-radiused Rc→Rc+d, wall band + 2 disc caps, planar-facet weld, no tessellator change; both gates — host π(Rc+d)²H + SIM vs OCCT `BRepPrimAPI_MakeCylinder` oracle: vol relO/relX ≤6.4e-3, area ≤2.4e-3, watertight, χ=2, bbox ≤2·defl, grow/shrink dir; OCCT-facade declines the curved wall as designed). Cone/sphere/stepped/multi-radius wall → OCCT |
