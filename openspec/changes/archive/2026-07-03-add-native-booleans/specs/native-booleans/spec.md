...
This change (Phase 4 #5, RESEARCH-GRADE) introduces the `native-booleans` capability: it
makes **`cc_boolean`** (`op`: `0` fuse, `1` cut `a−b`, `2` common) NATIVE in `NativeEngine`
for **PLANAR-faced solids** (polyhedra — boxes, prisms, convex or simple-concave) via a
four-stage pipeline (face–face intersection segments → face splitting → fragment
inside/outside/on classification → surviving-shell assembly + sew/heal to a watertight
`Solid`), matching OCCT EXACTLY on axis-aligned box cases. A **mandatory self-verify guard**
(valid + watertight 2-manifold + correct set-algebra volume) DISCARDS any bad native result
and falls through to OCCT. Curved-face solids, near-tangent / coincident / degenerate
configurations, and non-native operands fall through to OCCT (labelled, verified, never
faked). No `cc_*` ABI change; default engine stays OCCT.

> NOTE (honest scope): this is the **analytic-planar-first** slice of the research-grade
> boolean. A general robust B-rep boolean over arbitrary NURBS solids — surface–surface
> intersection curves, robust coincident/tangent handling, and full sew/heal — is future
> work. It is fully acceptable that only axis-aligned / planar-polyhedron booleans land
> native and everything else is OCCT-fallback; this spec states that split truthfully.
