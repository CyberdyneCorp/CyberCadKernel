#!/usr/bin/env bash
# Compile the MOAT ABI-parity (6 app-missing functions) native-vs-OCCT + closed-form parity harness
# #4b) for the iOS simulator and run it inside a booted simulator via
# `xcrun simctl spawn`.
#
# This is the profile-family sibling of run-sim-suite.sh (#4). It is
...
# test_native_loft + test_native_engine with plain `clang++ -std=c++20`; this
# script does NOT need it.
#
# Like run-sim-suite.sh, this harness drives the SHIPPING PATH: it calls
...
# always tests the current loft.h / NativeEngine / cc_set_engine sources. No
...
HARNESS="$REPO/tests/sim/native_abi_parity.mm"
...
echo "── compiling native-loft parity harness for iphonesimulator (arm64)"
...
xcrun simctl spawn "$UDID" "$OUT/native_abi_parity"
