...
- **S5 curved booleans** — using the traced WLines to split curved faces, classify fragments, and
  ...
  (S5 has since landed native slices S5-a/b/c — see the `native-booleans` **S5-a/b/c** result-table
  row — through-drill cyl∩cyl COMMON/FUSE/CUT + sphere∩sphere COMMON, native-pass=5.)
...
### Deferred to S4-c…f / later (recorded, not blocking the S4-a/b bar)
- **S4-c** — marching THROUGH a tangency / fabricating a curve across a degeneracy
  (near-tangent stepping, higher-order predictor). The hard core of the moat.
- **S4-d** — branch points (splitting the trace where branches meet/cross).
