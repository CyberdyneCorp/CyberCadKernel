#!/usr/bin/env bash
# Compile + run the MOAT M6-breadth-10 DIRECT-MODELING DIFFERENTIAL-FUZZING harness
# (tests/sim/native_directmodel_fuzz.mm) for the iOS simulator. This is the TENTH native
...
# native_geometry_services_fuzz.mm, native_transform_fuzz.mm and
# native_reference_geometry_fuzz.mm — to the DIRECT-MODELING layer the CyberCad app's
# direct-edit tools drive (cc_split_plane / cc_replace_face parallel-offset /
# cc_project_point_on_face). These three DM ops each have a per-op PARITY fixture
# (native_split_plane_parity / native_replace_face_parity / native_dm3_dm4_parity) but no
# *fuzz* domain — this closes that gap.
#
# Unlike the eight internal-C++ fuzzers (which link only the native library + an OCCT
# oracle slice), this harness — like native_hlr_parity — drives the SHIPPING PATH through
# the public cc_* facade under BOTH engines (cc_set_engine(0)=OCCT oracle,
# cc_set_engine(1)=NativeEngine) and compares the two results AND a THIRD engine-
# independent closed-form arbiter in plain fp64. So it must link the WHOLE kernel — facade
# + core + engine (NativeEngine + the OCCT adapter) + src/native/** — plus the full OCCT
# toolkit set (the OCCT adapter reaches booleans/half-space cut, meshing, mass/bbox
# queries, GeomAPI projection, STEP/IGES).
#
...
#
# NUMSCI IS REQUIRED. NativeEngine::split_plane / replace_face compose the freeform
# half-space cut whose seam trace is NUMSCI-only (guarded by CYBERCAD_HAS_NUMSCI in
# native_engine.cpp): WITHOUT the substrate the native DM path is compiled OUT and every
# split/offset trial honest-declines to OCCT, so the native SPLIT/OFFSET ops would never
# be exercised. So — like run-sim-native-split-plane.sh — this script builds the numsci
# iossim archive first and compiles the whole kernel under -DCYBERCAD_HAS_NUMSCI=1.
#
# Each op trial is classified AGREED / HONESTLY-DECLINED (native scoped-out, e.g. cone-
# lateral projection) / BOTH-DECLINED / ORACLE-INACCURATE (native matches exact math, OCCT
# is the outlier — native vindicated) / DISAGREED (a real finding) / ORACLE_UNRELIABLE
# (OCCT itself disagrees with the closed form — gated off). The bar: DISAGREED==0 AND
# ORACLE_UNRELIABLE==0, each base family + each op with >=1 AGREED. The generator is seeded
# ONLY by an explicit FUZZ_SEED (argv/env) — NO clock, NO rand(): same seed → byte-
# identical batch. On run-sim-suite.sh's SKIP list (own main(), std::_Exit).
#
# Usage: scripts/run-sim-native-directmodel-fuzz.sh [SEED] [N]
...
N="${2:-${FUZZ_N:-96}}"
if [ "${1:-}" != "" ]; then SEEDS=("$1"); elif [ "${FUZZ_SEED:-}" != "" ]; then SEEDS=("$FUZZ_SEED"); else SEEDS=("0xD3ADBEE710" "0x5EC0FFEE42"); fi

[ -d "$OCCT/include/opencascade" ] || { echo "OCCT sim install not found at $OCCT"; exit 1; }

HARNESS="$REPO/tests/sim/native_directmodel_fuzz.mm"
[ -f "$HARNESS" ] || { echo "missing harness: $HARNESS"; exit 1; }

# ── numsci iossim substrate (required by NativeEngine::split_plane's seam trace) ──────
...
# (construct prism/revolve, half-space split-plane cut, replace-face, GeomAPI projection,
# meshing, mass/bbox queries, and — since occt_exchange.cpp compiles in — STEP/IGES), so
# link the SAME broad toolkit set run-sim-suite.sh / run-sim-native-hlr.sh use (most-
# derived → base). TKHLR/TKShHealing keep the always-linked drafting/healing adapters
# satisfied.
...
echo "── compiling M6-breadth-10 DIRECT-MODELING differential-fuzz harness for iphonesimulator (arm64)"
...
for SEED in "${SEEDS[@]}"; do
  ...
  if ! xcrun simctl spawn "$UDID" "$OUT/native_directmodel_fuzz" "$SEED" "$N"; then rc=1; fi
done
exit $rc
