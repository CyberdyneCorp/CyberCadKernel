#!/usr/bin/env bash
# Compile the native-vs-OCCT HLR parity harness for the iOS simulator and
# run it inside a booted simulator via `xcrun simctl spawn`.
#
# This is MOAT M-GS GS1 (`native-drafting`) GATE (b) — the native-vs-OCCT
# hidden-line-removal parity pass (openspec/changes/moat-hlr-hidden-line-removal).
# GATE (a) (host analytic, no OCCT) is built by CTest as test_native_drafting with
# plain `clang++ -std=c++20`; this script does NOT need it.
#
# Unlike the earlier native_*_parity harnesses (which link only the native library
# + an oracle slice of OCCT and bridge a TopoDS_Shape into the native model), this
# harness drives the SHIPPING PATH: it calls the public cc_* facade under BOTH
# engines (cc_set_engine(0)=OCCT, cc_set_engine(1)=NativeEngine) and compares. So it
# must link the WHOLE kernel — facade + core + engine (NativeEngine + the OCCT
# adapter, which is the fallthrough target under CYBERCAD_HAS_OCCT) + src/native/**
# — plus the full OCCT toolkit set (the OCCT adapter reaches booleans, meshing,
# STEP/IGES, etc.).
#
...
# opencascade) rather than reuse a possibly-stale prebuilt static lib, so the
# harness always tests the current NativeEngine / cc_set_engine sources. No
...
# that suite's SKIP list — it carries its own main() and links the whole kernel +
...
HARNESS="$REPO/tests/sim/native_hlr_parity.mm"
...
# src/native/** (math TUs; topology/tessellate/construct are header-only). Same
...
# facade (construct/prism+revol, booleans, meshing, mass/bbox queries, and — since
# occt_exchange.cpp is compiled in — STEP/IGES), so link the SAME broad toolkit set
# run-sim-suite.sh uses (most-derived → base).
# TKHLR provides HLRBRep_Algo / HLRBRep_HLRToShape — the HLR oracle occt_drafting.cpp
# links (GATE b); it depends on the TKTopAlgo/TKGeomAlgo/TKBRep base already listed.
...
echo "── compiling native-HLR parity harness for iphonesimulator (arm64)"
...
xcrun simctl spawn "$UDID" "$OUT/native_hlr_parity"
