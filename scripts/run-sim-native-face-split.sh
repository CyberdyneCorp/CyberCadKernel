#!/usr/bin/env bash
# Compile the MOAT M2b / B2 (freeform face-split) native-vs-OCCT parity harness for the
# iOS simulator and run it in a booted simulator via `xcrun simctl spawn`.
#
# This is SIM GATE (b) of the two-gate B2 model. The native splitter
# (src/native/boolean/face_split.h, OCCT-FREE) partitions ONE trimmed freeform Bézier
# face along the real M1 seam (ssi::trace_intersection) into TWO genuinely-trimmed
# sub-faces; the M0 FaceMesher meshes them unchanged. The harness proves, against OCCT:
# per-sub-face watertight (2-manifold, vertices on the OCCT surface), the seam welds
# into one 2-manifold DISK (Euler χ=1), and areaIn+areaOut matches the OCCT BRepGProp
# area of the SAME parent trimmed face. Gate (a) (host, no OCCT) is
# tests/native/test_native_face_split.cpp.
#
# Like the S3 marching harness, the seam trace needs the substrate, so this script
...
# (Geom_BezierSurface + BRepBuilderAPI + BRepMesh + BRepGProp + GeomAPI project).
#
# B2 is INTERNAL — NO cc_* entry point; asserted at the cybercad::native C++ boundary.
set -euo pipefail
...
HARNESS="$REPO/tests/sim/native_face_split_parity.mm"
...
# BRepMesh_IncrementalMesh → TKMesh; BRepBuilderAPI_*/BRepLib → TKTopAlgo;
# BRepGProp/GeomAPI_ProjectPointOnSurf → TKGeomAlgo; Geom_BezierSurface → TKG3d;
# Geom2d_* → TKG2d; gp_*/GProp → TKMath; TKernel underneath. Most-derived → base.
# TKShHealing (ShapeAnalysis_*) is pulled in by TKMesh's wire visitor; place it after
# TKMesh and before the geometry base toolkits it depends on.
TKS="TKMesh TKShHealing TKGeomAlgo TKTopAlgo TKBRep TKGeomBase TKG3d TKG2d TKMath TKernel"
LFLAGS=""; for tk in $TKS; do LFLAGS="$LFLAGS -l$tk"; done

echo "── compiling B2 freeform face-split native-vs-OCCT parity harness (iphonesimulator arm64)"
...
xcrun --sdk iphonesimulator clang++ -target arm64-apple-ios16.0-simulator -isysroot "$SYSROOT" \
  ...
  -o "$OUT/native_face_split_parity"
...
xcrun simctl spawn "$UDID" "$OUT/native_face_split_parity"
