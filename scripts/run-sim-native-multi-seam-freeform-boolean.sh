#!/usr/bin/env bash
# Compile the MOAT M2-multiseam (multi-face corner-clip weld)
# native-vs-OCCT parity harness for the iOS simulator and run it in a booted simulator
# via `xcrun simctl spawn`.
#
# This is SIM GATE (b) of the two-gate M2-multiseam model. The native multi-face
...
HARNESS="$REPO/tests/sim/native_multi_seam_freeform_boolean_parity.mm"
...
echo "── compiling M2-multiseam multi-face corner-clip weld native-vs-OCCT parity harness (iphonesimulator arm64)"
...
xcrun --sdk iphonesimulator clang++ -target arm64-apple-ios16.0-simulator -isysroot "$SYSROOT" \
  ...
  -o "$OUT/native_multi_seam_freeform_boolean_parity"
...
xcrun simctl spawn "$UDID" "$OUT/native_multi_seam_freeform_boolean_parity"
