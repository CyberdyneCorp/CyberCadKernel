#!/usr/bin/env bash
# Compile the MOAT M2 freeform<->freeform closed-seam CUT/COMMON
...
# OCCT) is tests/native/test_native_freeform_freeform_multiseam.cpp.
#
...
HARNESS="$REPO/tests/sim/native_freeform_freeform_multiseam_parity.mm"
...
echo "── compiling L3-d Stage 5 multi-seam freeform sew native-vs-OCCT parity harness (iphonesimulator arm64)"
...
xcrun --sdk iphonesimulator clang++ -target arm64-apple-ios16.0-simulator -isysroot "$SYSROOT" \
  ...
  -o "$OUT/native_freeform_freeform_multiseam_parity"
...
xcrun simctl spawn "$UDID" "$OUT/native_freeform_freeform_multiseam_parity"
