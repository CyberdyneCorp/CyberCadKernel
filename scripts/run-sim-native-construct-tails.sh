#!/usr/bin/env bash
# Compile the native-vs-OCCT M7 construct-tails parity harness (moat-m7t, Phase 4
# #4b) for the iOS simulator and run it inside a booted simulator via
# `xcrun simctl spawn`.
#
# This is the sweep sibling of run-sim-native-loft.sh (Tier B). It is capability
# #4b (`native-construction`, Tier C — sweep / pipe-shell) verification gate 2 — the
# native-vs-OCCT parity pass (see openspec/NATIVE-REWRITE.md) for cc_solid_sweep
# (straight + smooth-arc paths native) and the deferred cc_twisted_sweep /
# cc_guided_sweep / cc_loft_along_rail (verified OCCT fall-through). Gate 1 (host unit
# tests, no OCCT) is built by CTest as test_native_sweep + test_native_engine with
# plain `clang++ -std=c++20`; this script does NOT need it.
#
# Like run-sim-native-loft.sh, this harness drives the SHIPPING PATH: it calls the
...
# adapter reaches booleans, meshing, STEP/IGES, etc.).
#
...
# always tests the current sweep.h / NativeEngine / cc_set_engine sources. No
...
HARNESS="$REPO/tests/sim/native_construct_tails_parity.mm"
...
# facade (construct/prism+revol+pipe, booleans, meshing, mass/bbox queries, and —
...
echo "── compiling native-sweep parity harness for iphonesimulator (arm64)"
...
xcrun --sdk iphonesimulator clang++ -target arm64-apple-ios14.0-simulator -isysroot "$SYSROOT" \
  ...
  -o "$OUT/native_construct_tails_parity"
...
xcrun simctl spawn "$UDID" "$OUT/native_construct_tails_parity"
