...
- ✅ **SSI Stage S1 (analytic surface-surface intersection) — DONE at the verification
  bar (both gates green), archived.** Change **`add-native-ssi-analytic`** (SSI-ROADMAP
  stage **S1**). SSI is the enabler for the S5 general curved-boolean payoff. S1 delivers
  ...
  closed form; NO GeomAPI / NO numsci). SSI is INTERNAL — no `cc_*` entry point; parity is
  asserted at the `cybercad::native::ssi` C++ boundary (like native-math / native-topology).
  **17 analytic-native pairs** verified vs OCCT `GeomAPI_IntSS` (all curve TYPES match;
  on-surface / coincidence residuals ≤ ~4e-15): plane∩plane (Line), plane∩sphere/⟂cyl/⟂cone
  (Circle), plane∠cyl/∠cone (Ellipse), plane∥cyl and parallel cyl∩cyl (2 Lines), plane∥gen-cone
  (Parabola), plane-steep-cone (2 Hyperbola), plane∩torus (⟂-axis 1–2 + ∋-axis 2 circles),
  sphere∩sphere (Circle), coaxial sphere∩cyl / sphere∩cone / cyl∩cone (Circles), coaxial
  cyl∩cyl (coincident). **Honestly DEFERRED → `NotAnalytic` (never faked):** skew cyl∩cyl
  (OCCT emits 7 Ellipse — a planar quartic, no degree-≤2 reduction), general cone∩cone,
  non-coaxial cone∩cyl / sphere∩cyl / sphere∩cone, oblique plane∩torus (spiric quartic),
  torus∩curved, all freeform pairs → **S2 subdivision seeding + S3 marching (both DONE)** / S4
  robustness. Both gates green: host `test_native_ssi` (**11 cases, 0 failed**; NUMSCI OFF
  CTest **23/23**, NUMSCI ON CTest **24/24**) + sim native-vs-OCCT `GeomAPI_IntSS` parity
  `scripts/run-sim-native-ssi.sh` (**18 pairs, 0 failed**). No regressions (`run-sim-suite.sh`
  **221/221**). Living change archived; see `openspec/SSI-ROADMAP.md` (S1 + S2 + S3 done; **S4 NEXT**).
- ✅ **SSI Stage S2 (subdivision seeding) — DONE at the verification bar (both gates green),
  TRANSVERSAL; near-tangent / coincident / degenerate is S4 (honest).** Change
  **`add-native-ssi-seeding`** (SSI-ROADMAP stage **S2**). Finds ≥1 seed point per TRANSVERSAL
  intersection branch for the FREEFORM (NURBS / Bézier / B-spline) and NON-CLOSED-FORM QUADRIC
  pairs S1 defers as `NotAnalytic`: recursive patch-AABB-overlap subdivision → candidate regions →
  `least_squares(S₁ − S₂ = 0)` refine on the numerics substrate → 3D/param dedup to ~one seed per
  branch. OCCT-free in `src/native/ssi/` (refine guarded by `CYBERCAD_HAS_NUMSCI`); INTERNAL (no
  `cc_*`). Both gates green: host `test_native_ssi_seeding` (**6 cases, 0 failed** — skew cyl→2,
  crossing spheres→1, sphere∩Bézier-bump→1, parallel planes→0, tangent spheres→`deferredTangent`
  (no faked seed), deeper resolution recovers a small loop; NUMSCI OFF CTest **23/23** with the
  NUMSCI-gated tests correctly ABSENT, NUMSCI ON CTest **25/25**) + sim native-vs-OCCT
  `GeomAPI_IntSS` **recall** parity `native_ssi_seeding_recall.mm`: **3/3 transversal branches
  recalled at recall 1.00**, tangent = 0 everywhere, max seed on-surface residual **3.51e-16** (via
  `GeomAPI_ProjectPointOnSurf::LowerDistance` on both OCCT surfaces, well under 1e-6). OCCT NbLines
  (3/2/2) is its arc-split count, not the analytic branch count the recall denominator uses. No
  regressions (`run-sim-suite.sh` **221/221**, xcframework rebuilt with `seeding.cpp`). **Honest
  scope:** TRANSVERSAL only — near-tangent / coincident / degenerate seeding ill-conditions the
  refine → deferred to **S4** (`SeedSet.deferredTangent`, never faked); completeness is a measured
  recall figure (`minPatchFrac` default 1/32 is the recall/cost knob). See `openspec/SSI-ROADMAP.md`.
- ✅ **SSI Stage S3 (marching-line tracer / WLine) — DONE at the verification bar (both gates green),
  TRANSVERSAL; near-tangent / coincident / branch-point marching is S4 (honest).** Change
  **`add-native-ssi-marching`** (SSI-ROADMAP stage **S3**). From each S2 seed, walks the intersection
  curve — predictor `t = normalize(n₁×n₂)`, adaptive step, **re-project** each node onto BOTH surfaces
  via the numerics substrate (`least_squares`), march both directions + stitch, until the curve
  closes (`Closed`) or exits a boundary (`BoundaryExit`); dedups retraced branches; fits a B-spline
  through the polyline → one WLine per transversal branch. OCCT-free in
  `src/native/ssi/{marching.h,marching.cpp}` (corrector / step / fit guarded by `CYBERCAD_HAS_NUMSCI`,
  `marching.cpp` an EMPTY TU with NUMSCI off); INTERNAL (no `cc_*`). Consumes the S2 `SeedSet`,
  produces a `TraceSet` of `WLine`s (each node carries (u1,v1,u2,v2)) — the S5 input contract. Both
  gates green: host `test_native_ssi_marching` (**7 cases, 0 failed** — crossing spheres / plane∩sphere
  / skew-cyl → Closed; ramp B-spline∩plane → BoundaryExit; tangent spheres → no curve (deferred, not
  faked); duplicate seed → 1 WLine; every node on both surfaces < 1e-6, fit error < 1e-3; NUMSCI OFF
  CTest **23/23** with the three NUMSCI-gated tests correctly ABSENT, NUMSCI ON CTest **26/26**) + sim
  native-vs-OCCT `IntPatch` / `GeomAPI_IntSS` **curve parity** `native_ssi_marching_parity.mm`:
  **5 pairs, 9 branches, 0 failed — all TRANSVERSAL fully-traced, 0 near-tangent-truncated**; branch
  counts match OCCT on every pair; **5/5 OCCT closed loops reproduced as Closed native WLines**
  (bspline∩plane correctly 0-closed / 4-open); worst on-OCCT-curve **1.60e-06**, on-surface **6.81e-07**
  (both skew-cyl-unequal), length delta **2.28e-03** abs / ~0.33% rel (bspline∩plane, within the
  deflection/step tol). No regressions (`run-sim-suite.sh` **221/221**; `marching.cpp` additive/guarded).
  **Honest scope:** TRANSVERSAL only — near-tangent branches are traced *up to* the tangent, marked
  `NearTangent`, counted in `nearTangentGaps` (never a point past it); coincident / branch-point /
  self-intersection marching deferred to **S4** (the moat, NEXT). `nearTangentGaps > 0` is the honest
  S4 hand-off signal. Feeds the S5 curved-boolean payoff. Living change archived; see
  `openspec/SSI-ROADMAP.md` (S1 + S2 + S3 done; **S4 robustness NEXT**, S5 curved booleans follows).
...
- ◐ **Tetrahedral VOLUME meshing (native-meshing) — kernel-only slice (GitHub #1).**
  Change **`add-native-tet-meshing`** — capability `native-meshing`. Adds an additive
  `cc_*` surface for FE volume meshing: `cc_tet_mesh(body, deflection, opts)`
  (tessellate the B-rep surface → fill the PLC) and `cc_tet_mesh_surface(verts, tris,
  opts)` (a raw closed TRIANGLE surface, OCCT-free), both emitting CalculiX **C3D4**
  (linear) / **C3D10** (quadratic) tets — nodes in `shape10tet` order, positive signed
  volume enforced, mid-edge nodes built NATIVELY (no TetGen `-o2`) — plus always-on
  native `cc_mesh_quality` (signed volume, dihedral angles, scaled Jacobian, aspect
  ratio; a regular tet scores 70.53° / scaledJ 1). New module `src/native/mesh/`:
  `quality.{h,cpp}` (ALWAYS-ON, pure geometry, no OCCT / no TetGen) + `tet_mesher.{h,cpp}`
  (the SOLE AGPL consumer). **The tet mesher is backed by the OPTIONAL, EXTERNAL,
  AGPL-3.0 TetGen** (`/home/leonardo/work/tetgen`, referenced by absolute path, NEVER
  vendored/committed), gated behind `CYBERCAD_HAS_TETGEN` (default OFF, mirroring
  `CYBERCAD_HAS_NUMSCI`): the default MIT build compiles/links **zero AGPL code** and
  `cc_tet_mesh` returns an empty mesh + `cc_last_error` ("tet meshing unavailable"),
  never crashing. `test_native_quality` runs in EVERY build; `test_native_tet` (a
  hardcoded cube → watertight C3D4/C3D10 at `pq1.4a…`, positive Jacobian, volume
  conservation, quality gate) is registered only under `CYBERCAD_HAS_TETGEN=ON`.
  Built via `scripts/build-tetgen.sh` (external sources → `build-tet/host/libtetgen_host.a`,
  predicates at -O0 / tetgen at -O2). **Honest scope:** this delivery is **kernel-only**
...
| 5·SSI | `add-native-ssi-analytic` (SSI-ROADMAP **S1**) | native-ssi | ✅ **DONE at the verification bar (both gates green), archived.** Analytic surface-surface intersection for the elementary family — OCCT-free header-only `src/native/ssi/`, over `src/native/math` only (IntAna-style closed form; NO GeomAPI / NO numsci). INTERNAL (no `cc_*`; parity at the C++ boundary). **17 analytic-native pairs** vs OCCT `GeomAPI_IntSS` (all TYPES match; onSurf/coin ≤ ~4e-15): plane∩plane (Line), plane∩sphere/⟂cyl/⟂cone (Circle), plane∠cyl/∠cone (Ellipse), plane∥cyl + parallel cyl∩cyl (2 Lines), plane∥gen-cone (Parabola), steep-cone (2 Hyperbola), plane∩torus (1–2 + 2 circles), sphere∩sphere (Circle), coaxial sphere∩cyl / sphere∩cone / cyl∩cone (Circles), coaxial cyl∩cyl (coincident). Host `test_native_ssi` **11 cases, 0 failed** (NUMSCI OFF CTest 23/23, NUMSCI ON 24/24) + sim `GeomAPI_IntSS` parity `run-sim-native-ssi.sh` **18 pairs, 0 failed**; no regressions (`run-sim-suite.sh` 221/221). **DEFERRED → `NotAnalytic` (never faked):** skew cyl∩cyl (OCCT 7 Ellipse — planar quartic), general cone∩cone, non-coaxial quadric pairs, oblique plane∩torus, torus∩curved, freeform → **S2 seeding (DONE)** / S3 / S4. Feeds the S5 curved-boolean payoff. |
| 5·SSI | `add-native-ssi-seeding` (SSI-ROADMAP **S2**) | native-ssi | ✅ **DONE at the verification bar (both gates green), TRANSVERSAL.** Subdivision seeding — finds ≥1 seed per TRANSVERSAL intersection branch for the FREEFORM (NURBS / Bézier / B-spline) and NON-CLOSED-FORM QUADRIC pairs S1 defers as `NotAnalytic`: recursive patch-AABB-overlap subdivision → candidate regions → `least_squares(S₁ − S₂ = 0)` refine on the numerics substrate → 3D/param dedup to ~one seed per branch. OCCT-free `src/native/ssi/{seed.h,patch_bounds.h,seeding.h,seeding.cpp}` (refine guarded by `CYBERCAD_HAS_NUMSCI`); INTERNAL (no `cc_*`). Host `test_native_ssi_seeding` **6 cases, 0 failed** (skew cyl→2, crossing spheres→1, sphere∩Bézier-bump→1, parallel planes→0, tangent spheres→`deferredTangent` no faked seed, deeper resolution recovers a small loop; NUMSCI OFF CTest **23/23** with the NUMSCI-gated tests correctly ABSENT, NUMSCI ON CTest **25/25**) + sim native-vs-OCCT `GeomAPI_IntSS` **recall** parity `native_ssi_seeding_recall.mm`: **3/3 transversal branches recalled at recall 1.00**, tangent=0 everywhere, max seed on-surface residual **3.51e-16** (`GeomAPI_ProjectPointOnSurf::LowerDistance` on both OCCT surfaces, ≪ 1e-6); OCCT NbLines 3/2/2 = its arc-split count, not the analytic branch count the recall denominator uses. No regressions (`run-sim-suite.sh` **221/221**, xcframework rebuilt with `seeding.cpp`). **Honest scope:** TRANSVERSAL only — near-tangent / coincident / degenerate → **S4** (`SeedSet.deferredTangent`, never faked); completeness is measured recall (`minPatchFrac` default 1/32 is the recall/cost knob). Feeds **S3 marching (DONE)** — the `SeedSet` is its input contract (one WLine per seed). |
| 5·SSI | `add-native-ssi-marching` (SSI-ROADMAP **S3**) | native-ssi | ✅ **DONE at the verification bar (both gates green), TRANSVERSAL.** Marching-line tracer (WLine) — from each S2 seed, predict `t = normalize(n₁×n₂)` → adaptive step → **re-project** onto BOTH surfaces via the numerics substrate (`least_squares`, m=n=4 well-posed with an along-tangent advance residual) → march both directions + stitch → `Closed` / `BoundaryExit` → dedup retraced branches → fit a clamped-uniform B-spline. OCCT-free `src/native/ssi/{marching.h,marching.cpp}` (corrector / step / fit guarded by `CYBERCAD_HAS_NUMSCI`, `marching.cpp` an EMPTY TU with NUMSCI off); INTERNAL (no `cc_*`). Consumes the S2 `SeedSet`, produces a `TraceSet` of `WLine`s (each node carries (u1,v1,u2,v2)) = the S5 input contract. Host `test_native_ssi_marching` **7 cases, 0 failed** (crossing spheres / plane∩sphere / skew-cyl → Closed; ramp B-spline∩plane → BoundaryExit; tangent spheres → no curve; dup seed → 1 WLine; nodes on both surfaces < 1e-6, fit < 1e-3; NUMSCI OFF CTest **23/23** tests ABSENT, NUMSCI ON CTest **26/26** adding #24/#25/#26) + sim native-vs-OCCT `IntPatch` / `GeomAPI_IntSS` **curve parity** `native_ssi_marching_parity.mm`: **5 pairs, 9 branches, 0 failed — all TRANSVERSAL fully-traced, 0 near-tangent-truncated**; branch counts match OCCT on every pair; **5/5 OCCT closed loops reproduced as Closed native WLines** (bspline∩plane correctly 0-closed / 4-open); worst on-OCCT-curve **1.60e-06**, on-surface **6.81e-07** (both skew-cyl-unequal), length delta **2.28e-03** abs / ~0.33% rel (bspline∩plane). No regressions (`run-sim-suite.sh` **221/221**, `marching.cpp` additive/guarded). **Honest scope:** TRANSVERSAL only — near-tangent traced up to the tangent (`NearTangent`, `nearTangentGaps`, never a point past it); coincident / branch-point / self-intersection → **S4** (the moat, NEXT). Feeds the S5 curved-boolean payoff. |
...
| 4 | `add-native-tet-meshing` (GitHub #1) | native-meshing | ◐ **kernel-only slice** — additive `cc_tet_mesh` / `cc_tet_mesh_surface` (CalculiX C3D4/C3D10, native mid-edge nodes, positive Jacobian) + always-on native `cc_mesh_quality` (signed vol / dihedral / scaled-Jacobian / aspect ratio). New `src/native/mesh/` (`quality.{h,cpp}` always-on; `tet_mesher.{h,cpp}` the sole AGPL consumer). Tet mesher backed by the **OPTIONAL, EXTERNAL, AGPL-3.0 TetGen** (absolute path, NOT vendored), gated `CYBERCAD_HAS_TETGEN` (default OFF, mirrors `CYBERCAD_HAS_NUMSCI`): default MIT build links ZERO AGPL, `cc_tet_mesh` returns empty + `cc_last_error` when OFF (never crashes). `test_native_quality` in every build; `test_native_tet` (cube → watertight C3D4/C3D10) gated ON. Built via `scripts/build-tetgen.sh`. Kernel-only (no FE patch test; CalculiX++ `CadMesher` wiring is a follow-up); closed-app shipping needs a TetGen commercial license. |
