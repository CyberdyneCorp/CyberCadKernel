...
> **✅ LANDED — MESH-COLLAR (the deflection-robust shared-seam-strip collar; the mesher weld band
> EXTENDED one refinement step, track `worktree-agent-ab8757b36ecc7604a`).** L3-BAND named the exact
> limiter: the shared-seam-strip collar inset `δ = min(0.5·segLen, 0.25·rSeam)` in
> `tessellate/seam_strip.h` SHRINKS with the deflection-driven seam segment length — as the mesh
> refines the seam is sampled more densely, `0.5·segLen → 0` (measured **δ = 4.3e-5** at d=0.002 on
> the asym fixture, `segLen ≈ 8.6e-5` for the r₂ ring of 26 562 samples), so the collar SUPPRESSION
> band eventually stopped covering the near-seam curvature Steiner pile each face's CDT still inserts
> along the near-vertical wall, and a shared seam edge got used 4× (the parity non-manifold whose
> count grew as the mesh refined). **The fix: tie `δ` to the SEAM RADIUS (`δ = 0.05·rSeam`,
> deflection-INDEPENDENT) instead of the shrinking `segLen`** — the collar ring placement and the
> suppression band share one inset (so the CDT fill and the spliced strip tile the annulus with no
> gap — never the naive sliver-drop that opens edges). On the asym pose (a=4 valley ∩ b=6 dome,
> r₁≈0.154 inner, r₂≈0.365 outer) the raw COMMON survivor histogram (`measure_multiseam_fine`, no
> orientation repair) goes **nonmanif 1 → 0 at d=0.002** (the OUTER-seam r₂ pile is now suppressed at
> that refinement); the survivor set stays stable (2) and the split gap stays exactly 0. **The mesher
> weld band is extended one refinement step.**
>
> **Byte-stability (structural).** The strip pass fires ONLY as a fallback (after the baseline +
> rim-pin passes fail to weld); every already-watertight mesh is byte-identical — verified: on this
> pose the strip does NOT fire at the welding d=0.0025 (COMMON) nor for CUT/FUSE (which weld through
> per-face CDT correspondence), and `test_native_seam_strip_weld` (FUSE d=0.0045 be=0 weld) +
> `test_native_freeform_freeform_multiseam` (9/9, symmetric fixture) stay GREEN. `git diff` touches
> ONLY `src/native/tessellate/seam_strip.h` (+ the regression test); `src/native` stays **OCCT-free**;
> no `cc_*` ABI; boolean/ UNTOUCHED.
>
> **The SHARPENED (deeper) residual — now two DISTINCT sub-band limiters, both never-leaky.** With the
> collar collapse removed, the end-to-end fine-deflection COMMON weld is gated by two residuals DEEPER
> than the collar-width wave, both OUT of this tessellate lane's reach and both HONEST-DECLINED to NULL
> by the verb's self-verify (never a silent wrong solid):
> - **d=0.002 — the annulus↔annulus collar-side WINDING collapse (VolumeInconsistent).** The raw
>   survivor mesh is now WATERTIGHT (be=0, nonmanif=0), but the two COMMON annulus survivors share the
>   inner+outer seams with material on OPPOSITE radial sides; the strip's per-loop collar-side vote
>   welds them into a watertight-but-SMALLER closed region (meshed V≈0.0037 vs closed-form 0.0069).
>   Caught by the two-sided VOLUME self-verify (`VolumeInconsistent`) — the coherent-orientation
>   selection that would pick the correct enclosing weld lives in `boolean/weldMultiCoherent` (takes
>   the FIRST watertight+coherent config, not the volume-matching one), OUT of this lane.
> - **d≤0.00125 — the over-dense-seam WELD-TOLERANCE merge (NotWatertight).** The seam is sampled
>   DENSER than the weld tolerance (`weldTol = 0.5·d = 6.25e-4` at d=0.00125 ≫ the r₁ seam spacing
>   ≈6.8e-5), so ~9 adjacent seam-ring vertices merge into one and a shared on-seam edge (measured
>   r=r₁, z=z*, dz=0) is used 4×. The strip mesh is non-watertight → the mesher returns the baseline →
>   the verb declines `NotWatertight`. Independent of collar width (residual invariant across
>   δ∈[0.01,0.25]·rSeam). Its genuine fix (decimate the registered seam ring to ≥2·weldTol spacing, or
>   a seam-density-aware weld tolerance) is the next MESH-COLLAR wave.
>
> Regression: `test_native_multiseam_asym::asym_fine_deflection_residual_is_mesher_not_assembly`
> now REGRESSION-GUARDS the fix — it proves (via the raw survivor histogram) the d=0.002 OUTER-seam
> non-manifold is CLEARED (nmOuter==0, nmInner==0, open==0, survivors==2), localizes the d=0.00125
> residual to the INNER seam, asserts the verb honest-declines `VolumeInconsistent` at d=0.002 and
> the in-band d=0.0025 STILL welds be=0. **Net: the collar is deflection-robust; the mesher weld band
> is extended; the remaining fine-deflection residual is sharpened into two distinct, layer-pinned,
> never-leaky honest declines — one in `boolean/` (winding), one still in `tessellate/` (weld-tol).**

> **✅ LANDED — MESH-WELD-TOL (the weld-resolution seam-ring decimation; the d≤0.00125
> over-dense-seam weld-tolerance merge is CLEARED, track `worktree-agent-a11569404514d21b4`).**
> MESH-COLLAR named the residual exactly, and the measurement SHARPENED its mechanism: at d=0.00125
> on the asym pose the inner seam ring carries 14 112 samples (spacing ≈6.8e-5 ≪ weldTol = 0.5·d =
> 6.25e-4 — up to ~13 samples per weld cell), and because the COLLAR ring mirrors the seam ring 1:1,
> the collar — where each face's CDT fill meets the spliced strip — is equally over-dense. The strip
> pass therefore went non-manifold at BOTH collar rings (measured: 6 edges at r₁+δ and r₂−δ, NOT on
> the seam), fell back to the baseline (whose 4×-used edges are the on-seam r=r₁ dz=0 signature
> MESH-COLLAR recorded), and the verb declined `NotWatertight`. **The fix (the named decimation
> option, `tessellate/seam_strip.h`): the registry is told the mesher's weld tolerance
> (`setWeldResolution`, same `max(0.5·d, 1e-7)` formula as the weld) and DECIMATES each registered
> seam ring — angle-ordered around the strip axis — to ≥ 2·weldTol spacing via a per-Entry `kept`
> flag (two points ≥ 2·tol apart can never share a weld cell of side tol; cell diagonal √3·tol).**
> The face mesher builds the strip rings AND the CDT collar loop from the KEPT subset only
> (`keptSeamVertex`), so the ring placement and the suppression band still share one inset and the
> CDT fill still terminates exactly on the spliced strip's collar — no gap, never a sliver-drop.
> Both faces resolve a seam sample to the SAME shared registry entry, so they keep the identical
> subset and the strips stay bit-identical. This is a TIGHTENING of what the weld may merge — never
> a widened tolerance (DISAGREED=0-safe): a ring already coarser than 2·weldTol keeps EVERY sample
> (bit-identical strip), and a ring that would decimate below 3 vertices keeps all. Sagitta cost of
> the coarser seam chord is (2·weldTol)²/(8·rSeam) = d²/(8·rSeam) ≪ d — inside the deflection band.
>
> **Measured (COMMON survivor solid, asym a=4 valley ∩ b=6 dome):** the raw survivor mesh at
> d=0.00125 goes **open=0 nonmanif=0 watertight** (was nonmanif=4 at the inner seam; inner ring
> decimated 14 112→751, outer 33 552→1 784), and stays clean through **d=0.0008** — the MESHER weld
> band no longer has a weld-tolerance ceiling. The verb verdict at d=0.00125 moves `NotWatertight`
> → `VolumeInconsistent` (wt=1, coh=1, be=0, meshed V≈0.0063 < closed-form 0.0069): **the sub-band
> residual is now ONE mechanism, not two** — the annulus↔annulus collar-side WINDING collapse in
> `boolean/weldMultiCoherent` (out of this tessellate lane), honest-declined to NULL by the
> two-sided volume self-verify at both d=0.002 and d=0.00125, never a leaky solid. In-band d=0.0025
> COMMON still welds `Ok` be=0.
>
> **Byte-stability.** The registry + strip code runs ONLY in the fallback strip pass; the baseline
> and rim-pin passes are code-path untouched. FNV byte-compare of the watertight operand meshes at
> the welding deflections d=0.005 and d=0.0025 is IDENTICAL before/after; `test_native_seam_strip_weld`
> (FUSE d=0.0045 fine-deflection weld), `test_native_freeform_freeform_multiseam` (9/9) and
> `test_native_tessellate` (17/17) stay green. One STALE pin was overturned by measurement (the same
> discipline BOOL-MULTISEAM applied to the symmetric fixture's curvature-mismatch claim):
> `test_native_seam_strip_weld::seamstrip_subthreshold_common_declines_never_leaks` pinned the
> CLEARED mechanism (`!watertight` + `NotWatertight` at SYM COMMON d=0.0018); measured post-fix the
> strip welds watertight there (wt=1, coh=1, be=0, vol≈0.00702 vs cf 0.00770) and the verb declines
> `VolumeInconsistent` — the case now pins the NEW mechanism, the never-leaky NULL invariant
> unchanged (this one test edit is the only out-of-lane-list file touched, forced by the required-
> green gate). `git diff` otherwise touches ONLY
> `src/native/tessellate/{seam_strip,face_mesher,solid_mesher}.h`
> + the owned regression test + this section; `src/native` stays **OCCT-free**; no `cc_*` ABI change;
> `boolean/`, `ssi/`, `math/` UNTOUCHED. Regression:
> `test_native_multiseam_asym::asym_fine_deflection_residual_is_mesher_not_assembly` now asserts the
> d=0.00125 raw histogram is fully manifold and watertight (nmInner==0, nmOuter==0, open==0,
> survivors==2) and that the verb honest-declines `VolumeInconsistent` (reaching the weld watertight,
> rep.watertight==1) at BOTH d=0.002 and d=0.00125. **Net: the weld-tolerance limiter is REMOVED from
> the mesher; the entire remaining sub-band decline is the single `boolean/weldMultiCoherent` winding
> residual — the next (boolean-lane) wave's target.**

> **✅ LANDED — BOOL-VOTE (the oracle-free divergence-volume weld gate; the "winding pick"
> narrative is OVERTURNED by measurement and the no-oracle silent-wrong is CLOSED, track
> `worktree-agent-a9de1742951a4acf5`).** MESH-WELD-TOL handed this wave "the single
> `boolean/weldMultiCoherent` winding residual" — select the volume-matching weld configuration
> instead of the first watertight one, and the sub-band `VolumeInconsistent` pins were expected to
> flip `Ok`. The measurement (`measure_multiseam_vote`, swept on the asym AND sym fixtures)
> OVERTURNS that mechanism: EVERY watertight+coherent orientation configuration encloses the SAME
> pinched volume (asym d=0.002: all 0.006172 vs the survivors' own divergence expectation 0.006619,
> −6.8%; sym d=0.0018: all 0.007019 vs 0.007448, −5.8%), because the loss is not an orientation
> pick at all — it is the shared seam-strip **COLLAR PINCH**: both annuli's per-loop collar-side
> votes are CORRECT and therefore IDENTICAL, the two faces splice the SAME strip triangles, the
> weld's coincident-duplicate-triangle drop annihilates BOTH copies, and the collar bands' volume
> (δ = 0.05·rSeam — deflection-INDEPENDENT) is lost. No enumerable configuration can restore it
...
> COMMON) — a shipped silent-wrong. **The fix (`boolean/weldMultiCoherent`): a weld configuration
> is accepted ONLY if it is watertight + coherent AND its enclosed volume agrees (±2%) with the
> ORACLE-FREE divergence-theorem expectation of that configuration — the survivor faces' summed
> signed per-face volumes through the same shared-EdgeCache baseline meshing (flipping a face
> negates its term).** Measured calibration: a legit weld only REPAIRS the seam pairing and drifts
> ≤ 2e-4 relative (asym d=0.0025 rim-pin 0.006554 vs 0.006553; sym d=0.002 baseline weld agrees
> exactly), vs the 5.8–6.8% pinch — the 2% band sits ≥ 100× above the repair drift and ≥ 2.9×
> below the smallest pinch; a NEW gate, nothing widened. No consistent configuration ⇒
> `VolumeInconsistent` with BOTH volumes as witnesses (`enclosedVolume` + the new
> `expectedVolume`), and watertight/coherent carried in the report — the decline map reads "the
> weld CLOSED; the VOLUME declined", not "the weld failed".
>
> Regression `test_native_multiseam_vote` (3/3): the no-oracle asym d=0.002 COMMON now DECLINES
> `VolumeInconsistent` with the witnesses (was: a wrong solid returned `Ok`); the in-band d=0.0025
> still welds with welded ≈ expectation (the gate does not over-fire); the expectation itself
> converges on the closed form (the gate measures the right quantity). The two sub-band pins are
> re-pinned to the measured mechanism and STRENGTHENED to assert BOTH call forms (with and without
> the oracle) plus the witnesses: `test_native_seam_strip_weld::seamstrip_subthreshold_common_
> declines_never_leaks` (sym d=0.0018) and `test_native_multiseam_asym::asym_fine_deflection_
> residual_is_mesher_not_assembly` (asym d=0.002 AND the KEPT d=0.00125 pin — the never-leaky
> property stays covered below the band). Suites green: multiseam_vote 3/3,
> freeform_freeform_multiseam 9/9, multiseam_asym 5/5, seam_strip_weld 2/2, nurbs_solid_boolean
> 8/8, boolean_readmit 5/5, multiseam_order_independence 1/1, tessellate 17/17
> (`test_native_feature_ops` fails 2 cases IDENTICALLY on main beafc55 — pre-existing, out of this
> lane). `git diff` touches ONLY `src/native/boolean/freeform_freeform_multiseam.h` + the owned
> tests/probe + this section; `src/native` stays OCCT-free; no `cc_*` ABI change
> (`MultiSeamCutReport` is header-internal; it gains `expectedVolume`); `ssi/`, `tessellate/`,
> `math/` UNTOUCHED. **Net: the boolean lane now DETECTS any material-moving weld oracle-free, so
