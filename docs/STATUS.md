...
| **STEP import (native slice, WIDENED)** (OCCT-free Part-21 reader for the AP203 elementary/B-spline subset + multi-solid `Compound` + B-spline-FACE round-trip + ELLIPSE-curve recognition, healed then self-verified watertight else → OCCT) vs OCCT `STEPControl_Reader` | `scripts/run-sim-native-step-import.sh` | **28 / 28** (native-written box vol rel 2.27e-16, cylinder rel 1.27e-3, holed-plate rel 2.90e-4, all watertight matching OCCT re-import; **FOREIGN OCCT-written box + cylinder imported NATIVELY match OCCT re-import EXACTLY, rel 0**; **multi-solid** foreign 2-solid file imports as a native Compound rel 2.14e-16 per-solid watertight; **B-spline-FACE** native solid round-trips EXACT vol 304.38; **ELLIPSE** curve recognised but an ellipse-on-quadric solid still declines → OCCT; **TOROIDAL_SURFACE** declines → OCCT (no native torus kind); native path provably exercised — per-face-oriented edge counts, not OCCT's unique count; host `test_native_step_reader` 12/12) |
...
| SSI **S5-a/b/c/d** curved boolean vs OCCT `BRepAlgoAPI` | `scripts/run-sim-native-ssi-curved-boolean.sh` | **18 / 18** (native-pass=12: drill cyl∩cyl COMMON/FUSE/CUT + **sphere∩sphere COMMON/FUSE/CUT eq+uneq (op-set COMPLETE 3/3, FUSE ΔV ≤ 8.3e-4 / CUT ΔV ≤ 9.3e-4 vs analytic+OCCT)** + **branched Steinmetz COMMON/FUSE/CUT (op-set COMPLETE 3/3: COMMON `16R³/3` volN=5.3287 ΔV=8.75e-4; FUSE volN=32.385 vs OCCT 32.366 ΔV=5.82e-4; CUT volN=13.526 vs OCCT 13.516 ΔV=7.22e-4)**, all wt, ΔV ≤ 9e-4; 6 honest fallbacks incl. sphere×box, cone×box) |
...
| **4 — Native rewrite** | math · topology · tessellation · construction · planar+box∩cyl booleans · planar blends · STEP export · numeric foundations (NumPP/SciPP) · SSI S1+S2+S3 · SSI S4-a/b (coincident-region + tangent-contact classification) · SSI S4-c (first near-tangent MARCH-THROUGH slice) · SSI S4-d (first branch-point slice: Steinmetz self-crossing localized + routed) · SSI S4-e (first chart-singularity slice: sphere parametric pole + cone apex crossed) · SSI S4-f (robust TRUE-RETURN loop-closure + self-intersection guard + adaptive completeness-critic re-seed — MEASURED recall floor, not a proof) · SSI S5-a/b/c/d (curved booleans: drill cyl∩cyl COMMON/FUSE/CUT + sphere∩sphere COMMON/FUSE/CUT (op-set COMPLETE 3/3) + branched Steinmetz COMMON/FUSE/CUT (op-set COMPLETE 3/3), native-pass=12) · **#6 curved fillet (circular cyl↔cap rim → rolling-ball TORUS canal, G1-tangent) · #7 wrap-emboss (rectangular pad on a cylinder lateral face)** · **shape-healing FIRST SLICE (tolerant sew + vertex/tolerance unification + degenerate removal + orientation fix — internal, verified vs OCCT `BRepBuilderAPI_Sewing`/`ShapeFix`)** · **STEP import (native slice, WIDENED) (OCCT-free Part-21 reader for the AP203 elementary/B-spline subset + foreign OCCT-written box/cylinder + multi-solid Compound + B-spline-FACE round-trip + ELLIPSE-curve recognition, healed + self-verified watertight else → OCCT; host round-trip 9/9 exact + sim parity 28/28)** | ◐ **substantially native (planar/analytic + SSI-driven curved booleans + S4 degeneracy classification + near-tangent march-through + first branch-point routing + sphere-pole/cone-apex chart-singularity crossing + robust closure/self-intersection + measured completeness floor + first curved-feature slices #6/#7 + shape-healing first slice + STEP import widened slice)**; curved tail (SSI S4-d general/freeform + S4-e general/freeform + S4-f general small-loop residual below the reached floor + self-intersection arc-splitting/topology repair · wider curved booleans/blends: general non-Steinmetz branched pairs + more curved-curved families (the equal-R orthogonal Steinmetz op-set is now COMPLETE 3/3 native) · CONCAVE/variable/non-circular-crease/cyl↔cyl fillets · deboss/non-rectangular/non-cylindrical wrap-emboss · **STEP import beyond the widened subset** (torus, ellipse-on-quadric solids, nested/transformed assemblies, AP242, complex/trimmed profiles, rational B-splines → OCCT; multi-solid Compound + B-spline-FACE round-trip now native; all IGES stays OCCT) · **healing RESIDUAL** (beyond-tol gaps, missing pcurves, self-intersecting wires, arbitrary broken industrial B-rep — the coincident-within-tolerance/degenerate/orientation first slice is native)) keeps OCCT linked; drop-occt (#8) BLOCKED (≈9–18 py) |
