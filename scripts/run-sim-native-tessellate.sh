...
# harness (tests/sim/native_tessellate_parity.mm) is the ONLY OCCT-dependent piece
...
HARNESS="$REPO/tests/sim/native_tessellate_parity.mm"
...
xcrun --sdk iphonesimulator clang++ -target arm64-apple-ios14.0-simulator -isysroot "$SYSROOT" \
  ...
  -o "$OUT/native_tessellate_parity"
...
xcrun simctl spawn "$UDID" "$OUT/native_tessellate_parity"
