...
- **Native core** (`src/native`) — OCCT-free C++20: `math` (vectors/transforms +
  ...
  while a branch saddle / genuine tangency STILL defers (no point fabricated past a degeneracy);
  plus **S5-a/b/c** — SSI-curve-driven
  ...
  the deeper **S4-d…f marching core** (branch points, singularities, self-intersection, deeper
  near-coincident bands) is the pending moat, and sphere fuse/cut + wider curved-curved families
...
bash scripts/run-sim-native-ssi-marching.sh  #  7/7  — SSI S3 marching tracer (5 transversal / 9 branches) + 2 S4-c cases vs OCCT IntPatch
