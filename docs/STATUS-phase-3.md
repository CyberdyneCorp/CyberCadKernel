...
- **VERIFIED here (iOS simulator, OCCT linked):** the Phase-3 suite
  (`scripts/run-sim-phase3-suite.sh`) is **70 passed, 0 failed, 0 deferred**, exit 0.
- **Full (property proven):** reference-geometry, robust wrap-emboss,
  robust thread-boolean, G2-blend, AND full-round-fillet all hit their real
  geometric bars.
- **Full-round-fillet now covers non-parallel planar walls too:** the true
  rolling-ball blend is proven for both the (anti-)parallel-wall strip AND the
  NON-PARALLEL dihedral case (genuinely non-parallel fixture, `n_L·n_R=-0.7241`,
  43.60° off-parallel) — valid + watertight (vol=628.5665), middle strip consumed,
  blend is a cylinder along the crease, axis equidistant (rL=rR=2.9541), and
  **G1-tangent to BOTH non-parallel walls** (cos(left)=cos(right)=1.000000, tol
  cos1°=0.999848). The **0 deferred** count reflects this: no planar case falls
  back any more. Only truly CURVED (non-planar) neighbours remain out of scope
  (fall back to a valid standard fillet by design).
...
| **full-round-fillet** | **full for ALL planar walls (parallel AND non-parallel dihedral)** | PARALLEL: Rolling-ball vol=782.8319 (== expect); middle face CONSUMED; blend is a cylinder; axis equidistant from both walls (dLeft=2.0126, dRight=2.0126, rTop=2.0126, axisX=0.0000); G1-tangent at BOTH seams dot(left)=dot(right)=1.000000 (tol cos1°=0.999848); deterministic v1==v2=782.831853, area a1==a2=619.398224; single-arg auto-detect matches. NON-PARALLEL dihedral (fixture n_L·n_R=-0.7241, 43.60° off-parallel — genuinely non-parallel): valid + watertight vol=628.5665; middle strip CONSUMED (no +Y face on original top plane); blend is a cylinder along the crease; axis equidistant rL=rR=2.9541 (cos(left)=cos(right)=1.000000); **G1-tangent to BOTH non-parallel walls** cos(left)=cos(right)=1.000000 (tol cos1°=0.999848). | Only truly CURVED (non-planar) neighbour faces stay out of scope: they fall back to a VALID standard edge fillet recorded deferred with the measured tangency gap. No planar case defers. Host stub: no-op `0`. |
...
bash scripts/run-sim-phase3-suite.sh   # expect: "70 passed, 0 failed, 0 deferred", exit 0
...
| Full-round rolling-ball blend consumes the middle face, cylinder blend, G1-tangent (dot=1.000000) both seams, deterministic — PARALLEL walls | phase3 suite: 10 checks + auto-detect |
| Full-round on NON-PARALLEL planar dihedral walls (fixture n_L·n_R=-0.7241, 43.60°) is valid + watertight (vol=628.5665), consumes the middle strip, blend is a cylinder along the crease, axis equidistant (rL=rR=2.9541), **G1-tangent to BOTH non-parallel walls** cos(left)=cos(right)=1.000000 (tol cos1°) | phase3 suite: non-parallel dihedral checks (PASS, not deferred) |
...
| Item | Why | Task |
|---|---|---|
| **Full-round for CURVED (non-planar) neighbours** | The rolling-ball solver is proven for ALL PLANAR walls (parallel AND non-parallel dihedral). Truly curved neighbour faces are out of scope by design: they fall back to a VALID standard edge fillet recorded deferred with the measured tangency gap. This is the only residual for full-round | `full-round-fillet` 4.5 (curved case) |
...
Reference-geometry, robust wrap-emboss, robust thread-boolean, G2-blend, AND
full-round-fillet are all **complete at the Phase-3 acceptance bar** (real
geometric property proven on the sim). Full-round-fillet now covers BOTH the
(anti-)parallel-wall strip AND the non-parallel planar dihedral case with a true
G1-tangent rolling-ball blend (dihedral fixture n_L·n_R=-0.7241, 43.60°; cos(left)=
cos(right)=1.000000; middle consumed; valid + watertight vol=628.5665). The only
residual is truly CURVED (non-planar) neighbours, which fall back to a valid
standard fillet by design. All five Phase-3 changes therefore flip to **✅** in
`ROADMAP.md`; Phase 3 is fully closed at the acceptance bar with that documented
curved-neighbour residual.
