#!/usr/bin/env bash
# Compile the MOAT M2 exact-NURBS face split by plane (L3-S1)
...
# (src/native/boolean/nurbs_plane_split.h, OCCT-FREE) composes B1 recognise → M1 seam trace
...
HARNESS="$REPO/tests/sim/native_nurbs_plane_split_parity.mm"
...
echo "── compiling L3-S1 exact-NURBS face split native-vs-OCCT parity harness (iphonesimulator arm64)"
...
xcrun --sdk iphonesimulator clang++ -target arm64-apple-ios16.0-simulator -isysroot "$SYSROOT" \
  ...
  -o "$OUT/native_nurbs_plane_split_parity"
...
xcrun simctl spawn "$UDID" "$OUT/native_nurbs_plane_split_parity"
