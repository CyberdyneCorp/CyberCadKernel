...
// Irreducible geometry (cylinder wrap + densify + loft + boolean) — systems band.
...
ShapeResult OcctEngine::wrap_emboss(EngineShape body, int faceId, const double* profileXY,
    ...
    return occt::occtGuard([&]() -> ShapeResult {
        ...
        // Wrap (arc-length px, axial py) onto the cylinder at radius r.
        ...
        const double rIn = boss ? R : R - depth;
        const double rOut = boss ? R + depth : R;
        if (rIn <= 1e-6) {
            return make_error("wrap_emboss: emboss depth exceeds radius");
        }
        // Densify: subdivide each profile edge by its arc (theta) span so the wrapped
        // wire FOLLOWS the cylinder instead of chording across it. Axial edges (dTheta=0)
        // stay single-segment. Capped at NMAX to bound the vertex count / loft time.
        const double chordTol = 0.5;  // points
        ...
        for (int i = 0; i < count; ++i) {
            ...
            for (int k = 0; k < n; ++k) {
                ...
                dense.push_back({ax + (bx - ax) * t, ay + (by - ay) * t});
            }
        }
        ...
        auto tryBuild = [&](const std::vector<std::pair<double, double>>& pts) -> TopoDS_Shape {
            BRepOffsetAPI_ThruSections gen(Standard_True /*solid*/, Standard_True /*ruled*/);
        ...
        TopoDS_Shape pad = tryBuild(dense);
        if (pad.IsNull()) {  // dense loft failed (high curvature) → coarse polygon (never regress)
        ...
        if (pad.IsNull()) {
            return make_error("wrap_emboss: could not loft the wrapped pad");
        }
