...
- **Native core** (`src/native`) — OCCT-free C++20: `math` (vectors/transforms +
  ...
  **CONE surface family opened — coaxial cone(frustum)∩cylinder COMMON** (`buildConeCylCommon`: a
  SINGLE S1-analytic seam circle where `r_c(s)=R0+s·tanα` equals `Rc`, apex-free; a frustum band
  welded to a cylinder-segment band along one pooled ring, closed by two disc caps — the
  min-radius-profile solid of revolution), verified vs a DUAL oracle — the closed form
  `V=V_frustum(rBot→Rc)+V_frustum(Rc→rTop)` AND OCCT (volN 19.107 vs analytic 19.111355 / OCCT
  19.111, ΔV 2.03e-04, ΔA 9.89e-05); sim native-pass=13; cone FUSE/CUT + transversal/apex cone
  pairs + coaxial cone∩sphere COMMON still defer → OCCT;
