...
- [x] 3.4 Parity tests: each `cc_*` vs the current in-app bridge behaviour. — RUNTIME COVERAGE DONE (**ios-run**): the full OCCT suite (`tests/sim/full_suite.cpp` + `checks_{construct,feature,booltransform,tessellate,query,exchange,accel}.cpp`, run via `scripts/run-sim-suite.sh`) exercises **all 57 `cc_*` entry points** in a booted simulator against analytic/round-trip references — **221 passed, 0 failed, exit 0**. Covers construct (extrude/revolve/loft/sweep/thread), features (fillet/chamfer/shell/offset/split), boolean + transforms, tessellate/face-meshes/edge-polylines, mass-props/moments/bbox/subshape-ids/chains, and STEP/IGES round-trip. The only remaining piece is a *direct byte-diff against the in-app `KernelBridge.mm`*, which requires the CyberCad app and is tracked under 6.2 (app link-swap).
