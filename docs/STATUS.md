...
| **#6 CURVED fillet** (circular cylinder↔plane rim → rolling-ball TORUS canal, CONVEX cap rim + CONCAVE boss-on-plate base rim) vs OCCT `BRepFilletAPI` | `scripts/run-sim-native-curved-fillet.sh` | **15 / 15** (REAL native `activeNative=1`: CONVEX 9/9 material-removing + CONCAVE 6/6 material-ADDING `grew=1`; vol rel ≤ 3.8e-3, area rel ≤ 2.1e-3, watertight, mesh-vol==B-rep; G1 cos=1.0 at both seams **analytic from the closed-form torus normal, not mesh-sampled**) |
...
| **STEP import (native slice, WIDENED)** (OCCT-free Part-21 reader for the elementary/B-spline subset + multi-solid `Compound` + B-spline-FACE round-trip + ELLIPSE-curve recognition + RIGID PLACED ASSEMBLIES + AP203/AP214/AP242 headers, healed then self-verified watertight else → OCCT) vs OCCT `STEPControl_Reader` | `scripts/run-sim-native-step-import.sh` | **33 / 33** (native-written box vol rel 2.27e-16, cylinder rel 1.27e-3, holed-plate rel 2.90e-4, all watertight matching OCCT re-import; **FOREIGN OCCT-written box + cylinder imported NATIVELY match OCCT re-import EXACTLY, rel 0**; **multi-solid** foreign 2-solid file imports as a native Compound rel 2.14e-16 per-solid watertight; **B-spline-FACE** native solid round-trips EXACT vol 304.38; **rigid PLACED ASSEMBLY** (transform tree) imports as a native placed Compound — 2-box: 2 solids, vol rel 3.74e-16, bbox Δ=0, faces 12/12; **AP214** header accepted (schema-independent reader); **ELLIPSE** curve recognised but an ellipse-on-quadric solid still declines → OCCT; **TOROIDAL_SURFACE** + non-rigid transforms + Form-B `MAPPED_ITEM` decline → OCCT; native path provably exercised — per-face-oriented edge counts, not OCCT's unique count; host `test_native_step_reader` 16/16) |
...
| **4 — Native rewrite** | math · topology · tessellation · construction · planar+box∩cyl booleans · planar blends · STEP export · numeric foundations (NumPP/SciPP) · SSI S1+S2+S3 · SSI S4-a/b (coincident-region + tangent-contact classification) · SSI S4-c (first near-tangent MARCH-THROUGH slice) · SSI S4-d (first branch-point slice: Steinmetz self-crossing localized + routed) · SSI S4-e (first chart-singularity slice: sphere parametric pole + cone apex crossed) · SSI S4-f (robust TRUE-RETURN loop-closure + self-intersection guard + adaptive completeness-critic re-seed — MEASURED recall floor, not a proof) · SSI S5-a/b/c/d (curved booleans: drill cyl∩cyl COMMON/FUSE/CUT + sphere∩sphere COMMON/FUSE/CUT (op-set COMPLETE 3/3) + branched Steinmetz COMMON/FUSE/CUT (3/3), native-pass=12) · **#6 curved fillet (circular cyl↔plane rim → rolling-ball TORUS canal, G1-tangent — CONVEX cap rim removes + CONCAVE boss-on-plate base rim ADDS material) · #7 wrap-emboss (rectangular pad on a cylinder lateral face)** · **shape-healing FIRST SLICE (tolerant sew + vertex/tolerance unification + degenerate removal + orientation fix — internal, verified vs OCCT `BRepBuilderAPI_Sewing`/`ShapeFix`)** · **STEP import (native slice, WIDENED) (OCCT-free Part-21 reader for the AP203 elementary/B-spline subset + foreign OCCT-written box/cylinder + multi-solid Compound + B-spline-FACE round-trip + ELLIPSE-curve recognition + RIGID PLACED ASSEMBLIES + AP203/AP214/AP242 headers, healed + self-verified watertight else → OCCT; host round-trip exact + sim parity 33/33)** | ◐ **substantially native (planar/analytic + SSI-driven curved booleans + S4 degeneracy classification + near-tangent march-through + first branch-point routing + sphere-pole/cone-apex chart-singularity crossing + robust closure/self-intersection + measured completeness floor + first curved-feature slices #6 (convex+concave circular fillet)/#7 + shape-healing first slice + STEP import widened slice)**; curved tail (SSI S4-d general/freeform + S4-e general/freeform + S4-f general small-loop residual below the reached floor + self-intersection arc-splitting/topology repair · wider curved booleans/blends incl. general non-Steinmetz branched · variable/non-circular-crease/cyl↔cyl-canal/blind-hole-rim fillets · deboss/non-rectangular/non-cylindrical wrap-emboss · **STEP import beyond the widened subset** (PMI, non-rigid/scaled/mirrored transforms, deep-nested assemblies, torus, ellipse-on-quadric solids, complex/trimmed profiles, rational B-splines → OCCT; multi-solid Compound + B-spline-FACE round-trip + rigid placed assemblies + AP214/AP242 headers now native; all IGES stays OCCT) · **healing RESIDUAL** (beyond-tol gaps, missing pcurves, self-intersecting wires, arbitrary broken industrial B-rep — the coincident-within-tolerance/degenerate/orientation first slice is native)) keeps OCCT linked; drop-occt (#8) BLOCKED (≈9–18 py) |
