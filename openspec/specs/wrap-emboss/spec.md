...
The robust wrap-emboss SHALL keep the exact existing `cc_wrap_emboss(CCShapeId
...
material when `boss != 0` or remove material when `boss == 0`). The robustness
improvement SHALL be an internal change to how the pad is built; no `cc_*`
signature or POD struct layout SHALL change, and the operation SHALL be at least
as capable as the previous `BRepOffsetAPI_ThruSections` implementation (never
regress).
