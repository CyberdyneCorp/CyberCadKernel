#!/usr/bin/env bash
# Compile the native-vs-OCCT PROFILE-construction parity harness (Tier A, Phase 4
# #4b) for the iOS simulator and run it inside a booted simulator via
# `xcrun simctl spawn`.
#
# This is the profile-family sibling of run-sim-native-construct.sh (#4). It is
# capability #4b (`native-construction`, Tier A) verification gate 2 — the native-vs-
# OCCT parity pass (see openspec/NATIVE-REWRITE.md) for the typed-profile / holed /
# profile-revolve ops. Gate 1 (host unit tests, no OCCT) is built by CTest as
# test_native_profile + test_native_engine with plain `clang++ -std=c++20`; this
# script does NOT need it.
#
# Like run-sim-native-construct.sh, this harness drives the SHIPPING PATH: it calls
# the public cc_* facade under BOTH engines (cc_set_engine(0)=OCCT,
# cc_set_engine(1)=NativeEngine) and compares. So it must link the WHOLE kernel —
# facade + core + engine (NativeEngine + the OCCT adapter, which is the fallthrough
# target under CYBERCAD_HAS_OCCT) + src/native/** — plus the full OCCT toolkit set
# (the OCCT adapter reaches booleans, meshing, STEP/IGES, etc.).
#
...
# always tests the current profile.h / NativeEngine / cc_set_engine sources. No
...
HARNESS="$REPO/tests/sim/native_construct_profiles_parity.mm"
...
echo "── compiling native-profile-construction parity harness for iphonesimulator (arm64)"
...
xcrun --sdk iphonesimulator clang++ -target arm64-apple-ios14.0-simulator -isysroot "$SYSROOT" \
  ...
  -o "$OUT/native_construct_profiles_parity"
...
xcrun simctl spawn "$UDID" "$OUT/native_construct_profiles_parity"
