#!/usr/bin/env bash
# Compile the MOAT M-DM DM2 (move a planar face to a target plane)
...
HARNESS="$REPO/tests/sim/native_replace_face_parity.mm"
...
echo "── compiling DM2 replace-face native-vs-OCCT parity harness (iphonesimulator arm64)"
...
xcrun --sdk iphonesimulator clang++ -target arm64-apple-ios16.0-simulator -isysroot "$SYSROOT" \
  ...
  -o "$OUT/native_replace_face_parity"
...
xcrun simctl spawn "$UDID" "$OUT/native_replace_face_parity"
