...
- **S5 curved booleans** — using the traced WLines to split curved faces, classify fragments, and
  ...
  row — through-drill cyl∩cyl COMMON/FUSE/CUT + sphere∩sphere COMMON/FUSE/CUT (op-set COMPLETE 3/3) + branched Steinmetz COMMON, native-pass=10.)
...
branched Steinmetz bicylinder **COMMON**, native-pass=10 — see the `native-booleans` **S5-a/b/c/d**
result-table row; sphere∩sphere FUSE/CUT are also now native (op-set COMPLETE 3/3), Steinmetz FUSE/CUT remain deferred → OCCT).
