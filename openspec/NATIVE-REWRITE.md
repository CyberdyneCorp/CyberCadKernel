...
- â **Numeric foundations (remaining-work #2 â the substrate) â NumPP/SciPP ADOPTED +
  ...
  *done-at-bar* and moving numeric foundations OFF the critical path. Deferred (NOT
  ...
  near-tangent MARCH-THROUGH slice S4-c + the BRANCH-POINT slices S4-d — the
  Steinmetz self-crossing bicylinder localized + routed, AND (M1) the FIRST FREEFORM branch point:
  a bicubic B-spline saddle ∩ plane-through-the-saddle X-crossing whose four OPEN arms (each
  branch-to-boundary) are now traced vs OCCT `GeomAPI_IntSS` (`branchPts=1 traced=4
  onSurf=5.1e-10`), the definite-contact bump control still ending with no arms — + the
  CHART-SINGULARITY slices
  ...
  general freeform branch points BEYOND the first slice (cusps / higher-multiplicity 3+-line
  junctions / saddle∩saddle that does not verify) + general/freeform + higher-order-cusp
  singularities + the S4-f general small-loop residual + self-intersection arc-splitting/topology
