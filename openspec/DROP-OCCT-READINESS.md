...
| shell | cc_shell | 1311 | **A** | resid M3 curved (cyl/cone/**sphere-cap** done) | 12 | convex-planar native; capped-cyl/cone-frustum native (M3cs); **sphere-cap dome / bowl native** (F2 `curved_shell.h`: concentric inner sphere Ri=Ro−t, outer+inner sphere walls + open-rim annulus, planar-facet weld, no tessellator change; both gates — host closed-form `seg(Ro,a)−seg(Ro−t,a)` + SIM vs OCCT `MakeThickSolid`+`BRepGProp`: vol relO/relX ≤6e-3, area ≤3.4e-3, watertight, shrank; hemisphere + shallow/deep caps). Non-convex-planar / box-with-curved-face / spherical-zone / freeform → OCCT (1312) |
| offset_face | cc_offset_face | 1321 | **A** | curved cyl+cone+sphere-wall done (M3co + F3) | 10 | planar face native (1322); **CYLINDER + CONE-FRUSTUM + SPHERE LATERAL WALL radial offset native** (`curved_offset.h`, planar-facet weld, no tessellator change): cyl re-radiused Rc→Rc+d (M3co); **cone-frustum → coaxial cone same σ, both cap radii shift d/cosσ** (F3); **sphere-cap dome → concentric sphere R→R+d, cap plane fixed** (F3). Both gates — host closed form (cyl π(Rc+d)²H, cone πH/3·(Rb'²+Rb'Rt'+Rt'²), sphere π(2R³/3−R²a+a³/3)) + SIM vs OCCT `MakeCylinder`/`MakeCone`/`MakeSphere` oracles (38/38: vol relO/relX cyl≤6.4e-3 / cone≤2.6e-3 / sphere deflection-bounded <2e-2, area ≤2.4e-3, watertight, χ=2, grow/shrink dir). Freeform / stepped / multi-radius / spherical-zone / off-centre wall → OCCT |
...
| **M3 curved-blend breadth** — ✅ MAJOR SLICES LANDED (fillet_face + full_round_fillet[_faces] + chamfer_face/chamfer_corner native via the spherical/planar corner weld; **curved-edge fillet_edges native on cylinder + cone-frustum + sphere cap-rims + the cyl↔cyl CANAL crossing-crease (Steinmetz bicylinder), assembly-layer, no tessellator change** — the app's @15 curved-fillet gap for every analytic-revolve substrate + the crossing-crease canal; **curved shell** capped-cyl/cone; **curved offset_face** capped-cyl LATERAL WALL radial offset (Rc→Rc+d, `curved_offset.h`, no tessellator change, both gates vs the OCCT `MakeCylinder` oracle); all two-gate vs OCCT) | **remaining:** fillet_edges_g2 (0 app sites, needs a G2 blend family), offset_face freeform/stepped/spherical-zone wall (cone-frustum + sphere-cap-dome wall now native, F3), torus/elliptical-crease + unequal-radius canal fillet (no OCCT-free native substrate / OCCT-built body), freeform-base wrap_emboss (needs freeform-surface breadth + a new tessellator weld); the cyl↔cyl canal arm is reached only once the native cyl-cyl boolean builds a bicylinder body via the facade (a BOOLEAN-track gap) | **M3** | 14 direct | **~1–3** (down from ~3–8; the freeform-boolean spine + corner/curved-rim/canal welds landed) |
