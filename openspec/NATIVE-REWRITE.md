...
untouched and every existing mesh is byte-identical). Still an honest DECLINE → OCCT: non-uniform/shear
transforms, deep-nested assemblies, PMI SEMANTICS, a PARTIAL/trimmed torus, and a
SURFACE_OF_REVOLUTION with no faithful native kind — an ELLIPSE / B-spline generatrix (general revolved
surface), a SKEW oblique line (hyperboloid) — plus foreign arbitrary-rational B-spline surfaces and
...
- â #8 `drop-occt` â planned; **NOT reachable** at the current native ceiling. Two
  ...
  revolution → native Kind::Torus), and the analytic-quadric SURFACE_OF_REVOLUTION reductions
  (cyl/cone/plane) (schema-independent,
  AP203/AP214/AP242 headers accepted); non-uniform/shear transforms, deep-nested assemblies,
  PMI semantics, complex/trimmed profiles, a PARTIAL/trimmed torus, and a general/ellipse revolution
  still fall back to OCCT. IGES import/export are DESCOPED — STEP-only). Until BOTH exist, OCCT stays linked
