#!/usr/bin/env bash
# Compile the MOAT M2 walled-bowl MID-WALL freeform half-space CUT
...
HARNESS="$REPO/tests/sim/native_walled_bowl_midwall_parity.mm"
...
xcrun --sdk iphonesimulator clang++ -target arm64-apple-ios16.0-simulator -isysroot "$SYSROOT" \
  ...
  -o "$OUT/native_walled_bowl_midwall_parity"
...
xcrun simctl spawn "$UDID" "$OUT/native_walled_bowl_midwall_parity"
