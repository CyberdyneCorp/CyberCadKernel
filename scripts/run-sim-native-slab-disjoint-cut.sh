#!/usr/bin/env bash
# Compile the MOAT M2b freeform<->analytic DISJOINT (multi-lump) CUT
...
HARNESS="$REPO/tests/sim/native_slab_disjoint_cut_parity.mm"
...
echo "── compiling M2b slab-disjoint-cut native-vs-OCCT parity harness (iphonesimulator arm64)"
...
xcrun --sdk iphonesimulator clang++ -target arm64-apple-ios16.0-simulator -isysroot "$SYSROOT" \
  ...
  -o "$OUT/native_slab_disjoint_cut_parity"
...
xcrun simctl spawn "$UDID" "$OUT/native_slab_disjoint_cut_parity"
