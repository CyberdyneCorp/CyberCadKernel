...
This change (Phase 4 #2, the SUBSTRATE + Extrema on-ramp slice) adds the `native-numerics`
capability in two honest, exactly-verifiable pieces. First, it **ADOPTS NumPP + SciPP**
(the org's C++20, MIT NumPy / SciPy ports) as the kernel's **OCCT-free numeric substrate** —
root-finding, nonlinear system solve, unconstrained minimization, least-squares, and dense
linear algebra — referenced by absolute path exactly like OCCT (NOT vendored) and built for
HOST + arm64-iOS-simulator, with SciPP's `special` + `stats` EXCLUDED per the Homebrew-libc++
ISO-29124 gap (`docs/EVAL-numpp-scipp.md`, verdict **GO WITH HARDENING**). Second, it adds a
thin OCCT-free `src/native/numerics/` facade over them (scalar root, `fsolve`, `minimize`
/BFGS, `least_squares`, dense `solve`/`lstsq`) and native **closest-point / projection** — the
nearest parameter `t` on a native curve and the nearest `(u, v)` on a native surface to a 3D
point, built on SciPP `minimize` / `least_squares` with multi-start seeding — matching OCCT
`Extrema` within tolerance.

> NOTE (honest scope): this is the **numeric substrate + the closest-point / projection
> on-ramp to SSI — it is NOT SSI and NOT curved booleans.** The hard surface-surface
> intersection core (marching, seeding / subdivision, near-tangent / coincident-surface
> robustness — OCCT `IntPatch` / `IntWalk` + BOPAlgo) is the NEXT capability (#5) and stays
> OCCT. The eval's near-tangent failures are the concrete evidence: from a naive seed generic
> `fsolve` found **0/7** freeform surface-surface intersection points (EXP2b), and a
> near-tangent configuration breaks BOTH Newton `fsolve` and damped Levenberg-Marquardt
> `least_squares` at d≥2.99 (EXP2c). Closest-point / projection, by contrast, is a
> well-conditioned single-target minimization the generic solvers handle cleanly (EXP1
> converged to err 7.4e-6), so it is squarely in scope. It is fully acceptable that ONLY the
> substrate + closest-point land here and SSI / curved booleans stay OCCT; this spec states
> that split truthfully. Full native SSI + curved booleans are what still block #8
> `drop-occt` — this change does NOT unblock it. No `cc_*` ABI change; the default engine
> stays OCCT.
