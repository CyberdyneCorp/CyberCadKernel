...
- ◐ Primitive & swept-solid construction (extrude/revolve/loft/sweep). Change
  ...
  *(CORE done at the verification bar — the first ENGINE-WIRED capability. Native
  ...
  a straight or smooth-planar spine), and Tier D (`cc_tapered_shank` + well-formed
  `cc_helical_thread` / `cc_tapered_thread` — the per-turn seam weld is fixed at the mesher
  level so threads mesh `boundaryEdges==0` at every deflection and run NATIVE) now DONE at
  the bar; still OCCT-fallthrough (not faked): non-planar / tight-curvature / real-twist /
  guided / rail sweep, a fine-pitch / self-intersecting thread (non-manifold regardless of
  weld → self-verify defers to OCCT `MakePipeShell`), wrap-emboss, 3+-section / guided /
  rail loft, arc/spline revolve.
