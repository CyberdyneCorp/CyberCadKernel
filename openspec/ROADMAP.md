...
- ◐ Primitive & swept-solid construction (extrude/revolve/loft/sweep). Change
  ...
  *(CORE done at the verification bar — the first ENGINE-WIRED capability. Native
  ...
  extrude + typed-profile revolve), Tier B (2-section ruled loft), and Tier C (sweep along
  a straight or smooth-planar spine) now DONE at the bar; still OCCT-fallthrough (not
  faked): non-planar / tight-curvature / real-twist / guided / rail sweep, threads,
...
| 4b | `add-native-swept-solids` (Tiers D–E) | native-construction (advanced) | ☐ follow-up — D threads / E wrap-emboss (+ 3+-section / guided / rail loft, non-planar / guided / rail sweep), currently OCCT-fallthrough |
