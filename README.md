...
- **Native core** (`src/native`) — OCCT-free C++20: `math` (vectors/transforms +
  ...
  `exchange` (native STEP AP203 EXPORT + IMPORT first slice for in-scope native solids + foreign OCCT-written box/cylinder), `ssi` (internal
...
| **blends: `cc_shell`** (uniform thickness, box-like planar solid — EXACT wall) | data exchange: **STEP IMPORT beyond the AP203 elementary/B-spline subset** (assemblies, AP242/PMI, complex profiles, bspline-face solids) + **IGES export/import** (all IGES stays OCCT / dropped per the earlier decision) |
...
| **exchange: `cc_step_import`** (native OCCT-free Part-21 reader for the AP203 elementary/B-spline subset the writer emits + foreign OCCT-written box/cylinder — two-pass tokenizer/mapper, healed + self-verified watertight, host round-trip 9/9 EXACT + sim OCCT parity `[NIMPORT]` 15/15 incl. foreign OCCT-written box/cylinder read natively at rel 0; any unsupported entity / assembly / non-mm / non-watertight → NULL → OCCT `STEPControl_Reader`) | exchange: arbitrary/AP242/PMI STEP + all IGES import — declines to NULL, engine falls through to OCCT (honest, never fabricated) |
...
bash scripts/run-sim-native-step-import.sh   # 15/15 — STEP import FIRST SLICE vs OCCT STEPControl_Reader (native-written box/cyl/holed-plate rel ≤ 1.3e-3 watertight; FOREIGN OCCT-written box/cyl read natively at rel 0)
```
