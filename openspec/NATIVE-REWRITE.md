...
compounds, AND single-level RIGID placed assemblies — schema-independent, AP203/AP214/AP242
headers all accepted; non-rigid/scaled transforms, deep-nested assemblies, PMI,
...
- â #8 `drop-occt` â planned; **NOT reachable** at the current native ceiling. Two
  ...
  flat multi-solid compounds, and single-level RIGID placed assemblies (schema-independent,
  AP203/AP214/AP242 headers accepted); non-rigid/scaled transforms, deep-nested assemblies,
  PMI, complex/trimmed profiles, and torus still fall back to OCCT. IGES import/export are DESCOPED — STEP-only). Until BOTH exist, OCCT stays linked
...
| 3 | STEP **import** (AP203/214/242 parse + reconstruct; **IGES DESCOPED — STEP-only**) â **NATIVE SLICE DONE + WIDENED at the bar**: OCCT-free Part-21 reader for the elementary/B-spline AP203 subset the native writer emits + foreign OCCT-written box/cylinder, healed via #4, self-verified watertight else -> OCCT; WIDENED to multi-solid `Compound` import + a native B-spline-FACE round-trip (exact) + ELLIPSE-curve recognition + single-level RIGID PLACED ASSEMBLIES (placed `Compound`, schema-independent AP203/AP214/AP242) (`[NIMPORT] 33/33`, host 29/29 exact; multisolid rel=2.14e-16, splineface exact, 2-box assembly vol rel=3.74e-16 bboxΔ=0). Residual -> OCCT: PMI, non-rigid/scaled/mirrored transforms, deep-nested assemblies, torus, ellipse-on-quadric solids, complex/trimmed profiles, rational B-splines, AP242 PMI. IGES import/export stay OCCT until `drop-occt`, then removed/stubbed (never native) | ~300â600k | uses #4 | done (widened subset) | 2â4 py |
