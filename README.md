...
- **Native core** (`src/native`) — OCCT-free C++20: `math` (vectors/transforms +
  ...
  **de-risks (does NOT unblock/complete) curved blends #6 + wrap-emboss #7**, whose seams are
  ...
  native-pass=10); Steinmetz FUSE/CUT still defer → OCCT;
  ...
  pending moat, and Steinmetz fuse/cut + general non-Steinmetz branched pairs +
...
bash scripts/run-sim-native-ssi-curved-boolean.sh # 18/18 — SSI S5-a/b/c/d curved booleans vs OCCT BRepAlgoAPI (native-pass=10: drill cyl∩cyl COMMON/FUSE/CUT + sphere∩sphere COMMON/FUSE/CUT (3/3) + branched Steinmetz COMMON; Steinmetz fuse/cut defer)
