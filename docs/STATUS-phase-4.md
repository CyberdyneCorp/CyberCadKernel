...
- **Capability #4 `native-construction` — done at the Phase-4 verification bar
  ...
  `add-native-concave-fillet` archived). Blind-hole bottom rim, variable radius, cyl↔cyl canal
  and non-circular creases stay OCCT.
...
## native-curved-fillet result table (#6 — CURVED-blend slice, CONVEX + CONCAVE)
...
`test_native_blend` **17 cases / 0 failed**; host CTest **29/29** (NUMSCI-OFF), **36/36**
(NUMSCI-ON).
...
under `cc_set_engine(0/1)`. **`[NCFILLET]` 15 passed / 0 failed** (`activeNative=1`, REAL
native — not a fallback: native `n` ≠ OCCT `o`). Native (n) vs OCCT `BRepFilletAPI` (o),
exact closed-form (x):
...
OCCT-fallthrough: blind-hole bottom rim, VARIABLE radius, cyl↔cyl / cyl↔cone canals,
NON-circular creases, freeform neighbours, convex `Rc < 2r`, seam-leaves-face, multi-edge,
segmented revolve rims.

### Files (#6 curved slice — convex + concave)
...
- `src/native/blend/curved_fillet.h` — `curved_fillet_edge(...)` (CONVEX cyl↔cap) +
  ...
  rim recognition by geometry, torus-canal quarter-tube + trimmed faces (wall + cap/annulus),
  planar-triangle facet soup (OCCT-free). The convex path is byte-identical (additive-only).
- `src/engine/native/native_engine.cpp` — `fillet_edges` tries planar (verify SHRINK) →
  ...
  each gated by its own correctly-signed `blendResultVerified`.
