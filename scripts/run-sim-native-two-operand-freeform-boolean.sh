#!/usr/bin/env bash
# Compile the MOAT M2-assembly / B4 (first freeform↔analytic half-space CUT)
...
# 6-face solid) and cuts it with BRepAlgoAPI_Fuse against a large box, then compares the
...
# (Geom_BezierSurface/Curve + BRepBuilderAPI sewing + BRepAlgoAPI_Fuse + BRepPrimAPI box +
...
HARNESS="$REPO/tests/sim/native_two_operand_freeform_boolean_parity.mm"
...
echo "── compiling M2-FUSE two-operand freeform boolean native-vs-OCCT parity harness (iphonesimulator arm64)"
...
xcrun --sdk iphonesimulator clang++ -target arm64-apple-ios16.0-simulator -isysroot "$SYSROOT" \
  ...
  -o "$OUT/native_two_operand_freeform_boolean_parity"
...
xcrun simctl spawn "$UDID" "$OUT/native_two_operand_freeform_boolean_parity"
