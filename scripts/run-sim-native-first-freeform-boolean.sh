...
HARNESS="$REPO/tests/sim/native_first_freeform_boolean_parity.mm"
...
xcrun --sdk iphonesimulator clang++ -target arm64-apple-ios16.0-simulator -isysroot "$SYSROOT" \
  ...
  -o "$OUT/native_first_freeform_boolean_parity"
...
xcrun simctl spawn "$UDID" "$OUT/native_first_freeform_boolean_parity"
