#!/usr/bin/env bash
# Compile the native-vs-OCCT CURVED-CHAMFER parity harness for the iOS simulator and run it
...
# fillet / planar offset-face / uniform shell) is exercised through the cc_chamfer_edges
...
HARNESS="$REPO/tests/sim/native_curved_chamfer_parity.mm"
...
echo "── compiling native-curved-chamfer parity harness for iphonesimulator (arm64)"
...
xcrun --sdk iphonesimulator clang++ -target arm64-apple-ios14.0-simulator -isysroot "$SYSROOT" \
  ...
  -o "$OUT/native_curved_chamfer_parity"
...
xcrun simctl spawn "$UDID" "$OUT/native_curved_chamfer_parity"
