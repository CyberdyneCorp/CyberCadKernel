#!/usr/bin/env bash
# Compile the SSI Stage-S2 (subdivision seeding) native-vs-OCCT PER-BRANCH RECALL
# parity harness for the iOS simulator, link it against the OCCT SSI oracle slice
# (GeomAPI_IntSS + Geom_* surfaces + GeomLProp_SLProps + point/curve projection) AND
# the NumPP/SciPP numsci archive (the least_squares refine), and run it inside a booted
# simulator via `xcrun simctl spawn`.
#
# This is the SSI-ROADMAP.md Stage-S2 verification gate 2 — the STRICT per-branch recall
# pass (native_ssi_seeding_parity.mm). It complements the seed-count recall harness
# run-sim-native-ssi-seeding.sh: this one matches each native seed to the ACTUAL OCCT
# intersection curve it lies on, and requires 100% recall of the TRANSVERSAL OCCT
# branches (tangential branches classified via ‖n₁×n₂‖ are excluded → S4). Gate 1
# (host, no OCCT) is tests/native/test_native_ssi_seeding.cpp.
#
# Like the seed-count recall harness (and unlike the S1 header-only harness), S2's
# refine needs the substrate, so this script:
...
#     GeomLProp_SLProps (surface normals for the transversal/tangential classification)
#     lives in TKGeomBase, already in the slice.
#
...
HARNESS="$REPO/tests/sim/native_ssi_seeding_parity.mm"
...
# ── OCCT oracle toolkits (same slice/order as the S1 harness) ─────────────────────
# GeomLProp_SLProps (surface normals for the transversal classification) is in TKGeomBase.
TKS="TKGeomAlgo TKBRep TKG3d TKGeomBase TKG2d TKMath TKernel"
LFLAGS=""; for tk in $TKS; do LFLAGS="$LFLAGS -l$tk"; done

echo "── compiling SSI-S2 seeding per-branch RECALL parity harness for iphonesimulator (arm64)"
echo "   harness : $HARNESS"
echo "   native  : ${#NATIVE_SRCS[@]} TU(s) (math + ssi/seeding + numerics) [NUMSCI]"
echo "   numsci  : $NUMSCI_LIB"
echo "   oracle  : OCCT GeomAPI_IntSS + Geom_* + GeomLProp_SLProps + ProjectPointOn{Surf,Curve}"
xcrun --sdk iphonesimulator clang++ -target arm64-apple-ios16.0-simulator -isysroot "$SYSROOT" \
  ...
  -o "$OUT/native_ssi_seeding_parity"
...
xcrun simctl spawn "$UDID" "$OUT/native_ssi_seeding_parity"
