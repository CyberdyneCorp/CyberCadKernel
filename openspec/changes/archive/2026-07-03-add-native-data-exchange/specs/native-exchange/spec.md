...
This change (Phase 4 #7, the EXPORT-ONLY native slice) adds the `native-exchange`
capability with ONE tractable, exactly-verifiable piece: it makes **`cc_step_export`**
NATIVE in `NativeEngine` for a **native-built, native-representable solid**. The native
writer walks the native `topology::Shape` (#2 `native-topology`), reads the attached
geometry (#1 `native-math`), and emits a valid **ISO 10303-21** (STEP AP203) exchange
file — the exchange header, the Part-42 B-rep entity graph (`CARTESIAN_POINT`,
`DIRECTION`, `AXIS2_PLACEMENT_3D`, `VERTEX_POINT`; `LINE` / `CIRCLE` /
`B_SPLINE_CURVE_WITH_KNOTS`; `EDGE_CURVE`, `ORIENTED_EDGE`, `EDGE_LOOP`, `FACE_OUTER_BOUND`
/ `FACE_BOUND`; `PLANE` / `CYLINDRICAL_SURFACE` / `CONICAL_SURFACE` / `SPHERICAL_SURFACE` /
`B_SPLINE_SURFACE`; `ADVANCED_FACE`, `CLOSED_SHELL`, `MANIFOLD_SOLID_BREP`), and the AP203
`ADVANCED_BREP_SHAPE_REPRESENTATION` + product / context wrapper — in **true millimetres**.
The written file MUST re-read through OCCT `STEPControl_Reader` to the SAME solid (volume /
bbox / topology within tolerance) — that is the mandatory correctness gate; a native file
that fails it is DISCARDED and the engine falls through to OCCT `STEPControl_Writer`. A
foreign / OCCT-built solid, or a native solid outside the representable Part-42 subset,
falls through to OCCT (labelled, verified, never faked). No `cc_*` ABI change; the default
engine stays OCCT.

> NOTE (honest scope): this is the **export-only native ceiling** for exchange. **STEP
> import (`cc_step_import`) stays OCCT** `STEPControl_Reader` (parsing an arbitrary
> ISO-10303-21 file — a full tokenizer + whole-schema entity resolver + B-rep
> reconstruction + healing of foreign files — is the large, long-lived part, out of
> scope). **IGES export/import (`cc_iges_export` / `cc_iges_import`) stay OCCT**
> `IGESControl_*` (a separate older format, out of scope). It is fully acceptable that
> ONLY STEP export of native-representable solids lands native and everything else is
> OCCT; this spec states that split truthfully. Full native STEP import + IGES + a
> general-curved kernel are what still block #8 `drop-occt` — this change does NOT unblock
> it.
