#!/usr/bin/env bash
# Compile the native-vs-OCCT CURVED-BOOLEAN parity harness for the iOS simulator and
# run it inside a booted simulator via `xcrun simctl spawn`.
#
# This is Phase 4 capability #5 (`native-booleans`) DEFERRED RESIDUAL #2 — the NARROW
# ANALYTIC curved-boolean slice (axis-aligned box ⟷ a cylinder whose axis is parallel
# to a box axis) — verification gate 2, the native-vs-OCCT parity pass (see
# openspec/NATIVE-REWRITE.md). The native analytic curved boolean (src/native/boolean/
# curved.h) is exercised through the cc_boolean facade under BOTH engines
# (cc_set_engine(0)=OCCT, cc_set_engine(1)=NativeEngine) and compared. Native intercepts
# axis-aligned box ⟷ axis-∥ cylinder cut(through+blind)/fuse(boss)/common (analytic
# volume boxVol±πr²·len, deflection-bounded), and everything outside the analytic slice
# (a NON-axis-aligned cylinder, a sphere, cone/NURBS/cylinder-cylinder/near-tangent)
# FALLS BACK to OCCT (labelled, verified, never faked). Gate 1 (host unit tests, no
# OCCT) is built by CTest as test_native_boolean / test_native_engine with plain
# `clang++ -std=c++20`; this script does NOT need it.
#
# Like the sibling native_boolean parity harness, this drives the SHIPPING PATH: the
...
# reaches booleans, meshing, mass/bbox queries, STEP/IGES, etc.).
#
...
# always tests the current NativeEngine / native-curved-boolean sources. No duplicate
...
HARNESS="$REPO/tests/sim/native_curved_boolean_parity.mm"
...
# (math TUs; topology/tessellate/construct/boolean are header-only). Same file set
...
# (construct/prism+revol, BOOLEANS (BRepAlgoAPI/BOPAlgo), meshing, mass/bbox queries,
# and — since occt_exchange.cpp is compiled in — STEP/IGES), so link the SAME broad
# toolkit set run-sim-suite.sh uses (most-derived → base).
...
echo "── compiling native-curved-boolean parity harness for iphonesimulator (arm64)"
...
xcrun --sdk iphonesimulator clang++ -target arm64-apple-ios14.0-simulator -isysroot "$SYSROOT" \
  ...
  -o "$OUT/native_curved_boolean_parity"
...
xcrun simctl spawn "$UDID" "$OUT/native_curved_boolean_parity"
