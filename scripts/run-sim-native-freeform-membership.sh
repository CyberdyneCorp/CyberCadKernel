#!/usr/bin/env bash
# Compile the native point-in-freeform-solid vs OCCT BRepClass3d parity harness for the iOS
...
# `xcrun simctl spawn`.
#
# This is MOAT M2c / B3 verification gate 2 (point-in-freeform-solid membership) — the
...
# harness (tests/sim/native_freeform_membership_parity.mm) is the ONLY OCCT-dependent piece
...
HARNESS="$REPO/tests/sim/native_freeform_membership_parity.mm"

# Native implementation TUs. Topology is header-only (nothing under
# src/native/tessellate to compile); the math library carries bspline/bezier .cpp.
...
xcrun --sdk iphonesimulator clang++ -target arm64-apple-ios14.0-simulator -isysroot "$SYSROOT" \
  ...
  -o "$OUT/native_freeform_membership_parity"
...
xcrun simctl spawn "$UDID" "$OUT/native_freeform_membership_parity"
